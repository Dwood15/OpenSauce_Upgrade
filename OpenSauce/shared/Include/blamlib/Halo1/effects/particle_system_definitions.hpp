...
namespace Yelo
{
	...
	namespace TagGroups
	{
		...
		}; static_assert(sizeof(s_particle_system_physics_constants) == 0x4, STATIC_ASSERT_FAIL); // max count: 16
		...
		}; static_assert(sizeof(s_particle_system_type_states) == 0xC0, STATIC_ASSERT_FAIL); // max count: 8
		...
		struct s_particle_system_type_particle_states
		{
			...
			TAG_TBLOCK(physics_constants, s_particle_system_physics_constants);
		}; static_assert(sizeof(s_particle_system_type_particle_states) == 0x178, STATIC_ASSERT_FAIL); // max count: 8
		...
		struct s_particle_system_types
		{
			...
			{
				TAG_FLAG(unused);
			}; static_assert(sizeof(__physics_flags) == sizeof(long_flags), STATIC_ASSERT_FAIL);
		...
		}; static_assert(sizeof(s_particle_system_types) == 0x80, STATIC_ASSERT_FAIL); // max count: 4
		...
		}; static_assert(sizeof(s_particle_system_definition) == 0x68, STATIC_ASSERT_FAIL); // max count: 1
	};
};