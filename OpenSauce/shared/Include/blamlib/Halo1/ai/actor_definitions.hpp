...
namespace Yelo
{
	...
	namespace TagGroups
	{
		struct s_actor_definition
		{
			...
			}; static_assert(sizeof(__more_flags) == sizeof(long_flags), STATIC_ASSERT_FAIL);
			...
			TAG_PAD(int32, 12);
		}; static_assert(sizeof(s_actor_definition) == 0x4F8, STATIC_ASSERT_FAIL); // max count: 1
		...
		struct s_actor_variant_change_colors
		{
			...
			TAG_PAD(int32, 2);
		}; static_assert(sizeof(s_actor_variant_change_colors) == 0x20, STATIC_ASSERT_FAIL); // max count: 4
		...
		}; static_assert(sizeof(s_actor_variant_definition) == 0x238, STATIC_ASSERT_FAIL); // max count: 1		
	};
};