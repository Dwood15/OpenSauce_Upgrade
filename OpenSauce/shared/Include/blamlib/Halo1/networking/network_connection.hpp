...
namespace Yelo
{
	...
	namespace Networking
	{
		...
		struct s_message_stream
		{
			...
			PAD24;
			PAD32;
		}; static_assert(sizeof(s_message_stream) == 0x534, STATIC_ASSERT_FAIL);
		...
		struct s_connection_message // made up name, nothing to verify name with
		{
			...
			byte* header_buffer;
			byte* data_buffer;
		}; static_assert(sizeof(s_connection_message) == 0x20, STATIC_ASSERT_FAIL);
		...
		}; static_assert(sizeof(s_connection_prioritization_buffer) == 0x10, STATIC_ASSERT_FAIL);
		...
		}; static_assert(sizeof(s_network_connection) == 0xA9C, STATIC_ASSERT_FAIL);
		...
		}; static_assert(sizeof(s_network_server_connection) == 0xAE4, STATIC_ASSERT_FAIL);
	};
};