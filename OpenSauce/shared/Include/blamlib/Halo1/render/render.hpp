...
namespace Yelo
{
	namespace Render
	{
		...
		}; static_assert(sizeof(s_render_window) == 0xAC, STATIC_ASSERT_FAIL);
		...
		struct s_render_fog
		{
			...
			UNKNOWN_TYPE(real);
		}; static_assert(sizeof(s_render_fog) == 0x50, STATIC_ASSERT_FAIL);
		...
		}; static_assert(sizeof(s_render_globals) == 0x9D298, STATIC_ASSERT_FAIL);
		...
		}; static_assert(sizeof(s_structure_render_globals) == 0x18, STATIC_ASSERT_FAIL);
		s_structure_render_globals* StructureRenderGlobals(); // defined in the implementing extension's code
	};

	namespace blam {
		void PLATFORM_API render_window(const uint16 local_player_index , void* render_camera , void* render_frustum, void* rasterizer_camera
			, void* rasterizer_frustum, void* rasterizer_target, const bool is_mirror);
	};
};