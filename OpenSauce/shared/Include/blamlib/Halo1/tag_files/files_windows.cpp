	...
	namespace blam {
		...
		s_file_reference& file_reference_create(s_file_reference& reference, cstring directory, cstring name, cstring ext, long_enum location) {
			...
			if (result > 0)
				...
				YELO_WARN("file_reference_create failed: %d; hint: @%p %s.%s", result, _ReturnAddress(), name, ext);
			}
		...
		//////////////////////////////////////////////////////////////////////////
		// files_windows.c
		uint32 PLATFORM_APIfile_get_position(const s_file_reference& reference) {
			reference.Verify();
		...
		bool PLATFORM_APIfile_set_eof(s_file_reference& reference, uint32 position) {
			reference.Verify();
