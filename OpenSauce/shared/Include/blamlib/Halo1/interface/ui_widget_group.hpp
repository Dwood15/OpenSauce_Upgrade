...
namespace Yelo
{
	...
	namespace TagGroups
	{
		struct ui_conditional_widget_reference
		{
			...
			PAD16;
			TAG_PAD(int32, 6);
		}; static_assert(sizeof(ui_conditional_widget_reference) == 0x50, STATIC_ASSERT_FAIL); // max count: 32
		...
		struct ui_child_widget_reference
		{
			...
			PAD16;
			TAG_PAD(int32, 5);
		}; static_assert(sizeof(ui_child_widget_reference) == 0x50, STATIC_ASSERT_FAIL); // max count: 32
		...
		}; static_assert(sizeof(ui_widget_definition) == 0x3EC, STATIC_ASSERT_FAIL);
	};
