...
namespace Yelo::Interface {
	...
	int32 MapListAddMapFromPath(cstring maps_path, cstring map_file_name, int32 system_map_index) {
		...
		s_map_list_map* entry = (s_map_list_map*)multiplayer_maps[mp_map_index];
...
namespace blam {
	...
	static Yelo::cstring k_map_list_mp_names_unicode_string_list_name = R"(ui\shell\main_menu\mp_map_list)";

	Yelo::cstring const k_map_list_ignored_map_names[10 + 1 + 3] = {
		"a10", "a30", "a50",
		"b30", "b40",
		"c10", "c20", "c40",
		"d20", "d40",
	...
	const Yelo::Interface::s_map_list_map_info k_map_list_mp_maps[] = {
		{0, "beavercreek", true},
		{1, "sidewinder", true},
		{2, "damnation", true},
		{3, "ratrace", true},
	...
	Yelo::int32    map_list_map_get_index(Yelo::cstring map_path) {
		char map_name[_MAX_FNAME] = "";
		auto map_file_type        = Yelo::Cache::GetMapNameFromPath(map_name, map_path);

		return Yelo::Interface::MapListMapGetIndexFromName(map_name);
	}

	bool map_list_should_ignore(Yelo::cstring map_path) {
		...
		for (Yelo::cstring name : k_map_list_ignored_map_names) {
		...
		int index = Yelo::Interface::MapListMapGetIndexFromName(map_name);
		return index != NONE;
	}
	...
	void      map_list_initialize() {
		...
		Yelo::Interface::MapListAddSystemMultiplayerMaps();

		// first add maps from the current working directory, then try adding from the user profile
		WIN32_FIND_DATA fd;
		Yelo::Interface::MapListAddMapTypessInPath(fd, "");

		// only try adding maps from the user profile if we detected the presence of an actual maps directory
		if (Yelo::Settings::UserProfileMapsPathExists()) {
			Yelo::cstring user_profile_path = Yelo::Settings::UserProfilePath();

			Yelo::Interface::MapListAddMapTypessInPath(fd, user_profile_path);
		}
	}

	void map_list_dispose() {
		Yelo::Interface::map_list_data_t multiplayer_maps = *Yelo::Interface::MultiplayerMaps();

		//YELO_ASSERT(multiplayer_maps != nullptr);

		for (int x = 0; x < multiplayer_maps.count; x++) {
			auto map = (Yelo::Interface::s_map_list_map *)multiplayer_maps[x];
			map->Dispose();
		}
