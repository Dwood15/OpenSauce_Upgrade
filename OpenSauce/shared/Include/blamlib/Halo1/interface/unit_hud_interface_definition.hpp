...
namespace Yelo
{
	...
	namespace TagGroups
	{
		struct auxilary_overlay_definition : public s_hud_element_overlay
		{
			...
			TAG_PAD(int32, 6);
		}; static_assert(sizeof(auxilary_overlay_definition) == 0x84, STATIC_ASSERT_FAIL); // max count: 16
		...
		struct auxilary_meter_definition
		{
			...
			TAG_PAD(int32, 6);
			TAG_PAD(int32, 16);
		}; static_assert(sizeof(auxilary_meter_definition) == 0x144, STATIC_ASSERT_FAIL); // max count: 16
		...
		struct unit_hud_interface_definition
		{
			...
			TAG_PAD(int32, 89);
			TAG_PAD(int32, 12);
		}; static_assert(sizeof(unit_hud_interface_definition) == 0x56C, STATIC_ASSERT_FAIL);
	};
};