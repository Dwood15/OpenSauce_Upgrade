	...
	namespace Cache {
		...
		bool c_map_file_finder::TryAndFind(_Out_ std::string& map_path, _Out_opt_ errno_t* return_access) {
			...
			if (return_access != nullptr) {
				*return_access=file_found ?  k_errnone : ENOENT; // File name or path not found.
			}
		...
		s_cache_file_data_load_state::s_cache_file_data_load_state(s_cache_header* cache_header, s_cache_tag_header* tag_header)
			: base_address(CAST_PTR(byte*, tag_header) + cache_header->tag_memory_size), memory_available(Enums::k_tag_allocation_size_upgrade - cache_header->tag_memory_size) { }

		byte* s_cache_file_data_load_state::Alloc(s_cache_tag_instance* tag_instance, size_t bytes) {
			byte* address=base_address;
			if (bytes > memory_available) { YELO_ASSERT_DISPLAY(false, "need to log the tag which we can't load here"); }
		...
		e_map_path_file_type GetMapNameFromPath(_Out_ char(&map_name)[_MAX_FNAME], cstring map_path) {
			...
			_splitpath_s(map_path, nullptr, 0, nullptr, 0, map_name, _countof(map_name), map_extension, _countof(map_extension));
