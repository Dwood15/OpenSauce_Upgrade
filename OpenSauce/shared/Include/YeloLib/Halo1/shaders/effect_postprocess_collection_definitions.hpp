...
namespace Yelo
{
	namespace TagGroups
	{
		...
		}; static_assert(sizeof(s_effect_postprocess_collection_script_variable) == 0x50, STATIC_ASSERT_FAIL);
		...
		struct s_effect_postprocess_collection_effect
		{
			...
			TAG_PAD(byte, 12);
		}; static_assert(sizeof(s_effect_postprocess_collection_effect) == 0x48, STATIC_ASSERT_FAIL);
		...
		}; static_assert(sizeof(s_effect_postprocess_collection) == 0x30, STATIC_ASSERT_FAIL);
	};
};