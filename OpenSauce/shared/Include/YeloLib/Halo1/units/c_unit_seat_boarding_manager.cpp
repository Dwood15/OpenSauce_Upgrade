...
#pragma region Unit Actions
				void c_unit_seat_boarding_manager::UnitApplyEffect(const datum_index unit_index, const datum_index effect, const tag_string& effect_marker) const {
					if (effect.IsNull()) { return; }

					blam::hs_effect_new_from_object_marker(effect, unit_index, effect_marker);
				}

				void c_unit_seat_boarding_manager::UnitApplyDamage(const datum_index unit_index, const datum_index target_unit_index, const datum_index damage_effect, const int16 region_index) const {
					if (damage_effect.IsNull()) { return; }
				...
				void c_unit_seat_boarding_manager::UnitExitFromSeat(const datum_index unit_index, const bool force_state, const bool force_exit) const {
					auto* unit_datum=blam::object_get_and_verify_type<s_unit_datum>(unit_index);
				...
				void c_unit_seat_boarding_manager::UnitEnterSeat(const datum_index unit_index, const datum_index target_unit_index, const int16 target_seat) const {
					...
					if (!target_seat_unit.IsNull()) { UnitExitFromSeat(target_seat_unit, false, true); }
...
#pragma region Animation
				...
				void c_unit_seat_boarding_manager::DoMountedKeyframeAction(const datum_index unit_index, const TagGroups::unit_mounted_state_keyframe_action& action) const {
					...
					for (int16 index=0; index < unit_definition.unit.seats.Count; index++) {
						...
						if (seated_unit_index.IsNull()) { continue; }
				...
				bool c_unit_seat_boarding_manager::StartAnimation(const datum_index unit_index, const s_unit_datum* unit_datum, const Enums::unit_seat_animation animation) const {
					// Get the animations for the unit's seat
					auto* animation_graph=GetObjectAnimations(unit_index);
					if (!animation_graph) { return false; }

					// Check that the animation graph has a matching seat entry
					if (unit_datum->unit.animation.seat_index >= animation_graph->units.Count) { return false; }
					...
					if (animation >= animation_seat.animations.Count) { return false; }
					...
					animation_index=blam::animation_choose_random_permutation_internal(Enums::_animation_update_kind_affects_game_state, unit_datum->object.animation.definition_index, animation_index);

					// Start the animation
					blam::object_start_interpolation(unit_index, 6);
					blam::unit_set_animation(unit_index, unit_datum->object.animation.definition_index, animation_index);
...
#pragma region State Update
				void c_unit_seat_boarding_manager::UpdateNonSeatedUnit(const datum_index unit_index) const {
					...
					if (TEST_FLAG(unit_datum.object.damage.flags, Flags::_object_body_depleted_bit)) { return; }
				...
				void c_unit_seat_boarding_manager::UpdateSeatedUnit(const datum_index unit_index) const {
					...
					if (seat_extension_definition->seat_boarding.Count != 1) { return; }

					auto& seat_boarding=seat_extension_definition->seat_boarding[0];
					if (!blam::unit_animation_state_interruptable(unit_datum.unit.animation, Enums::_unit_animation_state_yelo_seat_boarding)) { return; }
					...
					if (seat_boarding.boarding_type == Enums::_unit_seat_boarding_type_delayed) {
						...
						if (TEST_FLAG(seat_boarding.delay_until, Flags::_unit_seat_boarding_delay_until_flags_region_destroyed_bit) && (seat_boarding.region_index != NONE)) {
				...
				void c_unit_seat_boarding_manager::UpdateBoardingUnit(const datum_index unit_index) const {
					...
					if (TEST_FLAG(parent_unit_datum.object.damage.flags, Flags::_object_body_depleted_bit) && TEST_FLAG(seat_extension_definition.flags, Flags::_unit_seat_extensions_flags_exit_on_unit_death_bit)) {
						UnitExitFromSeat(unit_index, true);
						return;
					}
				}
				...
				void c_unit_seat_boarding_manager::UpdateMountedUnit(const datum_index unit_index) const {
					...
					for (int16 index=0; index < unit_definition.unit.seats.Count; index++) {
						...
						if (TEST_FLAG(seat_extension->flags, Flags::_unit_seat_extensions_flags_triggers_mounted_state_bit) && !GetUnitInSeat(unit_index, index).IsNull()) {
							is_mounted=true;
							break;
						}
					}
...
#pragma region Seat Access
				void c_unit_seat_boarding_manager::CanEnterSeat(const datum_index unit_index, const datum_index target_unit_index, const int16 seat_index, const bool is_targeted_seat, bool& result) const {
					auto& unit_datum=*blam::object_get_and_verify_type<s_unit_datum>(unit_index);
