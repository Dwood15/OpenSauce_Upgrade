	...
	namespace Objects
	{
		...
		void PLATFORM_API BipedSeatedMelee(const datum_index unit_index)
		{
			...
			if(weapon_index.IsNull())
			{
				...
				auto& animations_definition = *blam::tag_get<TagGroups::model_animation_graph>(unit_definition->object.references.animations.tag_index);

				auto animation_index = GetSeatWeaponAnimation(animations_definition
					, biped_datum->unit.animation.seat_index
					, biped_datum->unit.animation.seat_weapon_index
					, Enums::_weapon_class_animation_melee);
			...
			else
			{
				...
				melee_animation_length = (sbyte)blam::weapon_get_first_person_animation_time(weapon_index, 0, Enums::_first_person_weapon_animation_melee, NONE);
				melee_damage_keyframe = (sbyte)blam::weapon_get_first_person_animation_time(weapon_index, 1, Enums::_first_person_weapon_animation_melee, NONE);
			}
		...
		void UnitCanEnterSeatMultiteam(const datum_index unit_index
...
#pragma region Animation
		namespace Units { namespace Animations
		{
			...
			void PLATFORM_API AnimationStateDefinedKeyframe(const datum_index unit_index, const Enums::unit_animation_state state)
			{
				...
				{
					auto& unit_datum = *blam::object_get_and_verify_type<s_unit_datum>(unit_index);
					auto& animation_definition = *blam::tag_get<TagGroups::model_animation_graph>(unit_datum.object.animation.definition_index);
					auto& animation = animation_definition.animations[unit_datum.object.animation.state.animation_index];

					if(unit_datum.object.animation.state.frame_index == animation.key_frame_index)
					{
						handler(unit_index, Enums::_unit_animation_keyframe_primary);
					}
					else if(unit_datum.object.animation.state.frame_index == animation.second_key_frame_index)
					{
