...
namespace Yelo
{
	...
	namespace TagGroups
	{
		...
		struct unit_mounted_state_keyframe_action
		{
			...
			TAG_PAD(tag_block, 2);
		}; static_assert(sizeof(unit_mounted_state_keyframe_action) == 0x60, STATIC_ASSERT_FAIL);
		...
		struct unit_mounted_state
		{
			...
			TAG_PAD(tag_block, 2);
		}; static_assert(sizeof(unit_mounted_state) == 0x80, STATIC_ASSERT_FAIL);

		struct unit_extensions
		{
			TAG_TBLOCK(mounted_state, unit_mounted_state);
			TAG_PAD(tag_block, 4);
		}; static_assert(sizeof(unit_extensions) == 0x3C, STATIC_ASSERT_FAIL);
		...
		struct unit_seat_keyframe_action
		{
			...
			TAG_FIELD(tag_string, effect_marker);
			TAG_PAD(tag_block, 3);
		}; static_assert(sizeof(unit_seat_keyframe_action) == 0x98, STATIC_ASSERT_FAIL);
		...
		struct unit_seat_boarding
		{
			...
			TAG_PAD(tag_block, 2);
		}; static_assert(sizeof(unit_seat_boarding) == 0x70, STATIC_ASSERT_FAIL);
		...
		struct unit_seat_damage
		{
			...
			TAG_PAD(tag_block, 2);
		}; static_assert(sizeof(unit_seat_damage) == 0x88, STATIC_ASSERT_FAIL);
		...
		struct unit_seat_access
		{
			...
			PAD16;
			TAG_PAD(tag_block, 3);
		}; static_assert(sizeof(unit_seat_access) == 0x7C, STATIC_ASSERT_FAIL);
		...
		struct unit_seat_extensions
		{
			...
			TAG_PAD(tag_block, 4);
		}; static_assert(sizeof(unit_seat_extensions) == 0x64, STATIC_ASSERT_FAIL);
	};
};