...
namespace Yelo {
	namespace Configuration {
		...
		template<typename T>
		class c_configuration_value_list final : public c_configuration_list<c_configuration_value<T>> {
		...
		public:
			...
			c_configuration_value_list(const std::string& node_name, const T default_value) : c_configuration_list<c_configuration_value<T>>(node_name, [node_name, default_value]() {
				return c_configuration_value<T>(node_name, default_value);
			}) {}
		};
	};
};