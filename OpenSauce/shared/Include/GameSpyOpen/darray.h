...
typedef void (*ArrayElementFreeFn)(
	void	* elem
);
...
// like this as needed. As such, the allocated length will always be a multiple 
...
// is your chance to do any deallocation or cleanup required for the element 
...
// which would require knowledge of elements' structure, which the 
...
// any embedded pointers through that function. After calling this, the value 
// of what array is pointing to is undefined.
...
// element is passed by address, and the new element's contents are copied from 
...
// before being removed. All the elements after position n will be shifted over 
// to fill the gap. This function runs in linear time. It does not shrink the 
// allocated size of the array when an element is deleted; the array just 
// stays over-allocated.
...
#define NOT_FOUND -1	// This is returned when a search fails to find the key.
