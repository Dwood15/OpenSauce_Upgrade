...
void GOACryptInit(GOACryptState *state, unsigned char *key, unsigned char keysize)
{
    ...
    // Pass phrases may be used directly, with longer length compensating for 
	//	the low entropy expected in such keys.
    // Alternatively, shorter keys hashed from a pass phrase or generated 
	//	randomly may be used. For random keys, lengths	of from 4 to 16 bytes 
    ...
    rsum = 0;
	mask = 255;
	// FM: Original code did "i>=0", but when i==0 the statements inside the 
	// loop body have no effect.
    for (i=255;i>0;i--)
	{
    ...
    // Initialize the indices and data dependencies. Indices are set to 
	// different values instead of all to reduce what is known about the state 
	// of the state->cards when the first byte is emitted.
...
{
    // Picture a single enigma rotor with 256 positions, rewired on the fly by 
	// card-shuffling.
	
    // This cipher is a variant of one invented and written by Michael Paul 
	// Johnson in November, 1993.
    ...
    // Output one byte from the state in such a way as to make it very hard to 
	// figure out which one you are looking at.
	/*
...
void GOADecrypt(GOACryptState *state, unsigned char *bp, int len)
{
	...
	for (i=0; i<len; i++) {
		...
		// Output one byte from the state in such a way as to make it very hard 
