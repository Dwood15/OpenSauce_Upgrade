...
// This is an internal callback proxy for server list.
static void ListCallback(SBServerList *serverlist, SBListCallbackReason reason, SBServer server, void *instance)
{
	...
	case slc_serverupdated:
		if ((server->state & (STATE_BASICKEYS|STATE_FULLKEYS|STATE_VALIDPING)) == 0) // If the server was updated, but with no data, then the update failed!
...
// This is an internal callback proxy for the query engine.
...
#ifdef GSI_UNICODE
ServerBrowser ServerBrowserNewW(const gsi_char *queryForGamename, const gsi_char *queryFromGamename, const gsi_char *queryFromKey, int queryFromVersion, int maxConcUpdates, int queryVersion, SBBool lanBrowse, ServerBrowserCallback callback, void *instance)
{
	...
	UCSToAsciiString(queryForGamename, forGameName_A);
	UCSToAsciiString(queryFromGamename, fromGameName_A);
	UCSToAsciiString(queryFromKey, fromGameKey_A);
...
#ifdef GSI_UNICODE
SBError ServerBrowserUpdateW(ServerBrowser sb, SBBool async, SBBool disconnectOnComplete, const unsigned char *basicFields, int numBasicFields, const gsi_char *serverFilter)
{
	char serverFilter_A[1024];
	if (serverFilter != NULL)
		UCSToUTF8String(serverFilter, serverFilter_A);
...
#ifdef GSI_UNICODE
SBError ServerBrowserLimitUpdateW(ServerBrowser sb, SBBool async, SBBool disconnectOnComplete, const unsigned char *basicFields, int numBasicFields, const gsi_char *serverFilter, int maxServers)
{
	char serverFilter_A[1024];
	if (serverFilter != NULL)
		UCSToUTF8String(serverFilter, serverFilter_A);
...
static SBError WaitForTriggerUpdate(ServerBrowser sb, SBBool viaMaster)
{
	...
	while (sb->triggerIP != 0 && err == sbe_noerror)
	{
		msleep(10);
		err = ServerBrowserThink(sb);
		if (viaMaster && sb->list.state == sb_disconnected) // We were supposed to get info from master, but it's disconnected.
			break;		
	}
	return err;
...
#ifdef GSI_UNICODE
SBError ServerBrowserSendMessageToServerW(ServerBrowser sb, const gsi_char *ip, unsigned short port, const char *data, int len)
{
	char ip_A[128];
...
#ifdef GSI_UNICODE
SBError ServerBrowserSendNatNegotiateCookieToServerW(ServerBrowser sb, const gsi_char *ip, unsigned short port, int cookie)
{
	char ip_A[128];
	UCSToAsciiString(ip, ip_A);
...
SBError ServerBrowserConnectToServerWithSocket(ServerBrowser sb, SBServer server, SOCKET gamesocket, SBConnectToServerCallback callback)
{
    ...
    // For now, always do NAT negotiation.
...
SBError ServerBrowserAuxUpdateIPA(ServerBrowser sb, const char *ip, unsigned short port, SBBool viaMaster, SBBool async, SBBool fullUpdate)
{
	...
	if (!viaMaster) // Do an engine query.
	{
		...
		// Don't overwrite the existing update, otherwise the response for the 
		// first update will be mistaken as the response for the second update
...
#ifdef GSI_UNICODE
SBError ServerBrowserAuxUpdateIPW(ServerBrowser sb, const gsi_char *ip, unsigned short port, SBBool viaMaster, SBBool async, SBBool fullUpdate)
{
	char ip_A[128];
	UCSToAsciiString(ip, ip_A);
...
SBError ServerBrowserAuxUpdateServer(ServerBrowser sb, SBServer server, SBBool async, SBBool fullUpdate)
{
	...
	{
		// Remove this from the existing update lists if present.
		SBQueryEngineRemoveServerFromFIFOs(&sb->engine, server);
		SBQueryEngineUpdateServer(&sb->engine, server, 1, (fullUpdate) ? QTYPE_FULL : QTYPE_BASIC, usequerychallenge);
		viaMaster = SBFalse;
	} else // Do a master update.
...
#ifdef GSI_UNICODE
void ServerBrowserRemoveIPW(ServerBrowser sb, const gsi_char *ip, unsigned short port)
{
	char ip_A[128];
	UCSToAsciiString(ip, ip_A);
...
#ifdef GSI_UNICODE
const gsi_char *ServerBrowserErrorDescW(ServerBrowser sb, SBError error)
{
	GSI_UNUSED(sb);
...
#ifdef GSI_UNICODE
const gsi_char *ServerBrowserListQueryErrorW(ServerBrowser sb)
{
...
#ifdef GSI_UNICODE
SBServer ServerBrowserGetServerByIPW(ServerBrowser sb, const gsi_char *ip, unsigned short port)
{
	char ip_A[20];
...
#ifdef GSI_UNICODE
	GS_ASSERT(sortkey != NULL && _tcslen((const gsi_char *)sortkey) <= SORTKEY_LENGTH);
	_tcscpy(info.sortkey, (const gsi_char *)sortkey);
#else
...
#ifdef GSI_UNICODE
void ServerBrowserSortW(ServerBrowser sb, SBBool ascending, const gsi_char *sortkey, SBCompareMode comparemode)
{
	char sortkey_A[255];
	UCSToUTF8String(sortkey, sortkey_A);
...
{
	return (char *)inet_ntoa(*(IN_ADDR *)&sb->list.mypublicip);
}
...
// This allows the user to specify a broadcast address for LAN hosting.
...
----------------
Checks to see if NAT Negotiation is required, based on whether or not it's 
a LAN game, the public ip present, and several other factors. This returns an 
IP string to use for NatNeg, or if direct connection is possible.

Given the type of network connection you will make, here's what your game 
logic should do:
1.	For a LAN game, connect using ipstring.
2.	For an internet game, connect using ipstring.
3.	If NAT negotiation is required, perform it using the NatNeg SDK and this 
	ipstring before connecting.

This will return sb_true if further processing is required (i.e., NAT 
negotiation), and sb_false if it is not.
This fills an IP string.
*/
SBBool SBServerGetConnectionInfo(ServerBrowser gSB, SBServer server, gsi_u16 PortToConnectTo, char *ipstring)
{
	...
	{
            // Can directly connect to public IP, no negotiation is required.
			sprintf(ipstring,"%s:%d", SBServerGetPublicAddress(server),	SBServerGetPublicQueryPort(server) );
	}
	else
	{
		// NAT Negotiation is required.
		natneg = SBTrue;
		sprintf(ipstring,"%s:%d", SBServerGetPublicAddress(server),	SBServerGetPublicQueryPort(server) );
	}
	return natneg;
