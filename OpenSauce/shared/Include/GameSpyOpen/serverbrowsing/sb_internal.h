...
// Max number of bytes that can be received from a single recvfrom call (this
// must not be higher than 2048 for PS2 insock compatability):
...
// Length of time before a server query times out:
...
// Backend query flags (NOTE: this is set in hbmaster, don't change it):
...
// This keeps track of previous and current sorting information:
...
#ifdef GSI_UNICODE
const char *SBServerGetStringValueA(SBServer server, const char *keyname, const char *def); // This is for the Peer SDK.
