...
#define AUTHTEST_UNIQUENICK         _T("sctest01")
...
static void myCreateUserAccountCallback(GHTTPResult httpResult, WSCreateUserAccountResponse * theResponse, void * theUserData)
{
	if (httpResult != GHTTPSuccess)
		printf("* %s * Failed to create user, HTTP error: %d\n", (char*)theUserData, httpResult);
	else if (theResponse->mCreateUserAccountResult != WSLogin_Success)
		printf("* %s * Failed to create user, Login result: %d\n", (char*)theUserData, theResponse->mCreateUserAccountResult);  
	else //Create worked!
	{
		_tprintf(_T("* %s * UniqueNick '%s' created!\n"), (char*)theUserData, theResponse->mCertificate.mUniqueNick);
		gNumPassed++;
...
static void myCreateUserAccount(void)
{
	gsi_u32 result;

	result = wsCreateUserAccount(WSLogin_PARTNERCODE_GAMESPY, WSLogin_NAMESPACE_SHARED_UNIQUE, 
		AUTHTEST_EMAIL, AUTHTEST_NICK, AUTHTEST_UNIQUENICK, AUTHTEST_PASSWORD, myCreateUserAccountCallback, _T("wsCreateUserAccount"));
...
#else //#if _PS3 || _PSP || _PSP2

// PS3/PSP/PSP2 specific globals
#if defined(_PS3)
...
#elif defined(_PSP2)
SceSize gTicketLen = 0;
SceUChar8 *gTicket = NULL;
SceNpAuthRequestId gRequestId = 0;
...
void cleanupNP()
{
	...
	ret = sceNpAuthDestroyRequest(gRequestId);
	if (ret < 0)
	{
		printf("Unable to Destroy Request! Result = 0x%x", ret);
	}
...
static int auth_callback( SceNpAuthRequestId id, int result, void *arg)
{
	...
	/* result is negative if this transaction failed */
	if(result < 0)
	{
		printf("%s: Transaction Failed! Result = 0x%x\n", __FUNCTION__, result);
		return 0;
	}

	/* obtain ticket */
	gTicketLen = result;
	gTicket = (SceUChar8 *)malloc(gTicketLen);
	if(gTicket == NULL)
	{
		printf("%s: could not malloc ticket space.\n", __FUNCTION__);
		return 0;
	...
	ret = sceNpAuthGetTicket(id, gTicket, gTicketLen);
	if(ret < 0)
	{
		printf("sceNpAuthGetTicket() failed. ret = 0x%x\n", ret);
		return 0;
...
gsi_bool retrieveNpTicket(const char * serviceId, gsi_u8 ** ticketData, size_t * ticketSize)
{
	...
	while(!gGotTicket)
	{
		...
		if ((current_time()-startTime) > AUTHTEST_TIMEOUT)
		{
			printf("NP Ticket Request timed out. NP Ticket was not retrieved.\n");
			return gsi_false;
		}

		// poll to check for callback
		sceNpCheckCallback();
		msleep(10);
	}
	
	// Pass ticket info to parameters.
	*ticketData = gTicket;
	*ticketSize = (size_t) gTicketLen;
...
void RunLoginTests(int argc, char *argv[])
{
	...
	// NP Ticket can be freed after invoking wsLoginSonyCert() function
	if (npTicket)
		gsifree(npTicket);
