...
typedef enum WSLoginValue
{
	...
	WSLogin_DBError,
	WSLogin_ServerError,
	WSLogin_FailureMax,         // This must be the last failure.

	// Login result (mLoginResult):
	WSLogin_HttpError = 100,    // Ghttp reported an error, response ignored.
	WSLogin_ParseError,         // Couldn't parse http response.
	WSLogin_InvalidCertificate, // Login was successful, but the certificate was invalid!
	...
	WSLogin_UnknownError,       // An error occured, but detailed information is unavailable.

	// Response codes dealing with errors in response headers:
	WSLogin_InvalidGameID = 200, // Make sure the GameID is properly set with wsSetGameCredentials.
	WSLogin_InvalidAccessKey,    // Make sure the Access Key is properly set with wsSetGameCredentials.
...
#define WS_LOGIN_UNIQUENICK_LEN       (50+1)
...
// A user's login certificate, signed by the GameSpy AuthService. The 
// certificate is public and may be freely passed around. Avoid use of pointer 
// members so that the structure may be easily copied.
...
// -- NOTE: Be careful! Private key information must be kept secret. --
...
typedef void (*WSLoginPs3CertCallback)(
	GHTTPResult				httpResult, 
	WSLoginPs3CertResponse	* response, 
	void					* userData
);
typedef void (*WSLoginSonyCertCallback)(
	GHTTPResult				httpResult, 
...
//wsLoginValueString
// Summary
//		Given a WSLoginValue, this returns a meaningful string describing the 
//		login result.
// Parameters
//		loginValue		: [in] The login value
// Returns
//		A nul-byte terminated character string describing the login result 
...
// Summary
//		Set your Access Key and Secret Key, which will be passed with every web 
//		service call for authentication and usage/metric tracking.
// Parameters
//		accessKey		: [in] The Access Key provided via the GameSpy 
//							Developer portal.
//		secretKey   	: [in] The Secret Key provided by the GameSpy 
//							Developer portal.
// Returns
//
// Notes
...
COMMON_API WSCreateUserAccountValue wsCreateUserAccount(
	int								partnerCode, 
	int								namespaceId, 
...
//		profileNick		: [in] The nickname associated with the user's GameSpy 
//								account.
//		email			: [in] The email associated with user's GameSpy 
//								account.
//		password		: [in] The password for the user's GameSpy account.
//		cdkeyhash		: [in] CD key hash.
//		callback		: [in] Pointer to a function that will be called by the 
//								SDK to report the result of the authentication 
//								request.
//		userData		: [in] A pointer to data that will be supplied to the 
//								callback function.
// Returns
//		WSLoginValue: If successful, the value WSLogin_Success will be 
//		returned. Otherwise, a code specific to the error encountered will be 
//		returned.
// Notes
//		The GameSpy SDK Core must be initialized first using gsCoreInitialize 
//		before using this function.
...
//wsLoginUnique
// Summary
//		Login using a subset of the GameSpy Presence login information, 
//		requiring only a unique nick and password.
// Parameters
//		gameId			: [in] The game id.
//		partnerCode 	: [in] The partner code.
//		namespaceId		: [in] The namespace ID.
//		uniqueNick 		: [in] The unique nickname associated with the user's 
//								GameSpy account.
//		password		: [in] The password for the user's GameSpy account.
//		cdkeyhash		: [in] CD key hash.
//		callback		: [in] Pointer to a function that will be called by the 
...
//wsLoginRemoteAuth
// Summary
//		Login by authenticating with a partner system and then use that 
//		authentication information with the GameSpy system.
// Parameters
//		gameId				: [in] The game id.
//		partnerCode 		: [in] The partner code.
//		namespaceId			: [in] The namespace ID.
...
//									the SDK to report the result of the 
//									authentication request.
//		userData			: [in] A pointer to data that will be supplied to 
...
typedef struct WSLoginFacebookResponse{
    WSLoginValue	mLoginResult;	// Result of the login attempt.
...
typedef void (*WSLoginFacebookCallback)(
	GHTTPResult					httpResult, 
...
//wsLoginFacebook
// Summary
//		Generates authentication credentials to use with GameSpy Remote 
//		Authentication based on an active Facebook session.
// Parameters
//		gameId 		    : [in] The game id for your title.
//		uid			    : [in] The uid returned from a successful login via 
//								Facebook Connect.
//		apiKey 			: [in] The apiKey associated with your Facebook 
//								application.
//		sessionKey	    : [in] The sessionKey returned from a successful login 
//								via Facebook Connect.
//		sessionSecret	: [in] The sessionSecret returned from a successful 
...
//								callback function.
// Returns
//		WSRequest: If successful, the created GSTask will be returned, which 
//		can be used to cancel the request with gsiCoreCancelTask if necessary. 
//		Otherwise, NULL will be returned.
// Notes
//		The GameSpy SDK core must be initialized first using gsCoreInitialize 
//		before using this function. This is an iPhone-specific function, and 
//		requires the use of the Facebook Connect Libraries for iPhone.
// See Also
...
COMMON_API WSLoginValue wsLoginPs3Cert(
	int						gameId, 
	int						partnerCode, 
	int						namespaceId, 
...
//wsLoginSonyCert
// Summary
//		Login for the PS3 or PSP system; authenticates the NP account and 
//		creates a corresponding GP 'shadow account'.
// Parameters
//      gameId 		    : [in] The game id for your title.
//		partnerCode 	: [in] The partner code.
//		namespaceId		: [in] The namespace ID.
...
//		The GameSpy SDK core must be initialized first using gsCoreInitialize 
//		before using this function.
COMMON_API WSLoginValue wsLoginSonyCert(int gameId, int partnerCode, int namespaceId, const gsi_u8 * ps3cert, int certLen, WSLoginSonyCertCallback callback, void * userData);
...
COMMON_API gsi_bool wsLoginCertWriteXML(
const GSLoginCertificate	* cert, 
const char					* anamespace, 
GSXmlStreamWriter			writer
);
COMMON_API gsi_bool wsLoginCertWriteBinary(
	const GSLoginCertificate	* cert, 
	char						* bufout, 
	unsigned int				maxlen, 
	unsigned int				* lenout
);
COMMON_API gsi_bool wsLoginCertReadBinary(
	GSLoginCertificate	* certOut, 
	char				* bufin, 
	unsigned int		maxlen
);
COMMON_API gsi_bool wsLoginCertReadXML(
	GSLoginCertificate	* cert, 
	GSXmlStreamReader	reader
);

COMMON_API char* wsGetServiceUrl(
);
...
typedef struct Buddy{
    int mProfileid;		// The profileid associated with buddy.
...
typedef void (*WSGetBuddyListCallback)(
	GHTTPResult				httpResult, 
...
//wsGetBuddyList
// Summary
//		Grabs the names and profileids for a user's buddy list using an 
//		authenticated login certificate.
// Parameters
//		gameId 		    : [in] The game id for your title.
//		certificate	    : [in] The user's authenticated login certificate.
//		privData 		: [in] The private data returned with authentication.
//		callback		: [in] Pointer to a function that will be called by the 
//								SDK to report the result of the request.
//		userData		: [in] A pointer to data that will be supplied to the 
//								callback function.
// Returns
//		WSRequest: If successful, the created GSTask will be returned, which 
...
typedef void (*WSSyncFacebookCallback)(
	GHTTPResult				httpResult, 
...
//wsSyncFacebook
// Summary
//		Grabs the names, profileids, and Facebook UIDs for a Facebook user's 
//		buddy list using an authenticated login certificate, after first 
//		syncing up the Facebook friends list with any registered Facebook 
//		accounts in the GP system.
// Parameters
//		gameId 		    : [in] The game id for your title.
//		certificate	    : [in] The user's authenticated login certificate.
//		privData 		: [in] The private data returned with authentication.
...
//		The GameSpy SDK core must be initialized first using gsCoreInitialize 
//		before using this function. This is an iPhone-specific function, and 
//		requires usage of the Facebook Connect Libraries for iPhone.
// See Also
