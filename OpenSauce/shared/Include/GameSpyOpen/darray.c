...
#else
	#include "nonport.h" // This is for gsimalloc/realloc/free/GS_ASSERT.
#endif
...
{
	GS_ASSERT(array->elemsize != 0);	// Sanity check.
...
{
	GS_ASSERT(array != NULL);			// Safety check.
	GS_ASSERT(elem != NULL);	
...
void ArrayRemoveAt(DArray array, int n)
{
	...
	if (n < array->count - 1) // If this is not last element.
...
// mybsearch
//		Implementation of a bsearch, since it's not available on all platforms.
