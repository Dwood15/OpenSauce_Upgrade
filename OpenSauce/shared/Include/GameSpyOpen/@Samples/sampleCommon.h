...
// Use ATLAS-specific sample game credentials here:
#ifdef ATLAS_SAMPLE 
...
#else
// Game identifiers, which you can find on your Developer Dashboard on 
...
void SampleAuthenticationCallback(
	GHTTPResult		httpResult, 
	WSLoginResponse	* theResponse, 
	void			* theUserData
);

gsi_bool SampleAuthenticatePlayer(
...
SAKE sake;
gsi_bool SakeSetup();
const char * SakeFieldValueToString(
	SAKEField	* field
);
void DisplaySakeRecords(
	SAKEField ** records, 
	int numRecords, 
	int numFields
);
SAKEGetMyRecordsOutput * CopySakeRecordsFromResponseData(
	SAKEGetMyRecordsInput  * input, 
	SAKEGetMyRecordsOutput * output
);
void FreeSakeRecords(
	SAKEGetMyRecordsOutput	* records, 
	int						numberOfFields
);
gsi_bool DeleteSakeRecord(
	char	* tableName, 
	int		recordId
);
void SakeCleanup();
...
#ifdef ATLAS_SAMPLE
gsi_bool AtlasReportingSetup(
	SCInterfacePtr	* statsInterface, 
	int				playerIndex, 
	char			* sessionId
);
gsi_bool AtlasQuerySetup(
	SCInterfacePtr	* statsInterface
);
void DisplayAtlasPlayerRecords(
