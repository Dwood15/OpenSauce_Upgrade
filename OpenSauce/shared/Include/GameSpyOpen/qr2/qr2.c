...
void qr2_shutdown(qr2_t qrec)
{
	...
	// We only free the NatNeg Negotiator list if we used ACE.
	if (qrec->cc_callback)
		NNFreeNegotiateList();
...
//#define SIMULATE_FIREWALL
	...
	// When this qr2 client is registered with the master server, it will get
	// this message from the master server.
