...
int ClientUniqueID;
#else
static int ClientUniqueID;
#endif

static int GetClientUniqueID(void)
{
	if(ClientUniqueID == 0)
	{
		Util_RandSeed((unsigned long)current_time());
		ClientUniqueID = Util_RandInt(1, GSI_MAX_I32);
	}

    //gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_State, GSIDebugLevel_Notice,
    //    "GetClientUniqueID: Client ID = %d\n", ClientUniqueID);
...
static void SendPreinitPacket(NATNegotiator neg)
{
	NatNegPacket p;

	memcpy(p.magic, NNMagicData, NATNEG_MAGIC_LEN);
	p.version = NN_PROTVER;
	p.packettype = NN_PREINIT;
	p.cookie = (int)htonl(neg->cookie);
	p.Packet.Preinit.clientindex = (unsigned char)neg->clientindex;
	p.Packet.Preinit.clientID = GetClientUniqueID();

	gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
		"Sending PREINIT to %s:%d\n", inet_ntoa(*(IN_ADDR *)&matchup1ip), MATCHUP_PORT1);

	SendPacket(neg->negotiateSock, matchup1ip, MATCHUP_PORT1, &p, PREINITPACKET_SIZE);

	if(neg->state == ns_preinitack)
	{
		neg->retryTime = current_time() + PREINITACK_RETRY_TIME;
		neg->maxRetryCount = PREINITACK_RETRY_COUNT;
	}
	else
	{
		neg->retryTime = current_time() + PREINIT_RETRY_TIME;
		neg->maxRetryCount = PREINIT_RETRY_COUNT;
		neg->state = ns_preinitsent;
	}
}
    ...
    void *userdata)
{
    ...
    // For ACE, send pre-init packets to tell the NAT server to use queueing 
	// functionality. If useQueue is false, or if we are using the 
	// backwards-compatible function calls, we just move straight to the init 
	// packet phase and ignore the queueing mechanism altogether.
    if (useQueue)
	    SendPreinitPacket(neg);
    else
	    SendInitPackets(neg);
...
static void NegotiateThink(NATNegotiator neg)
{
	...
	if (neg->state == ns_preinitsent || neg->state == ns_preinitack ||
		neg->state == ns_initsent || neg->state == ns_connectping) //see if we need to resend packets
	{
		if (current_time() > neg->retryTime)
		{
			if (neg->retryCount > neg->maxRetryCount)
			{
				...
				if(neg->state == ns_preinitsent || neg->state == ns_preinitack || neg->state == ns_initsent)
			...
			else
			{
				...
				if (neg->state == ns_preinitsent || neg->state == ns_preinitack) // Resend preinit packet.
					SendPreinitPacket(neg);
				else if (neg->state == ns_initsent) // Resend init packets.
					SendInitPackets(neg);
				else
...
static void ProcessPreinitAckPacket(NATNegotiator neg, NatNegPacket *p)
{
	gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
		"Got preinit ack with state %d\n", p->Packet.Preinit.state);

	if(neg->state != ns_preinitack)
	{
		neg->state = ns_preinitack;
		neg->progressCallback(neg->state, neg->userdata);
		neg->retryTime = current_time() + PREINITACK_RETRY_TIME;
		neg->retryCount = 0;
		neg->maxRetryCount = PREINITACK_RETRY_COUNT;
	...
	switch(p->Packet.Preinit.state)
	{
	case NN_PREINIT_WAITING_FOR_CLIENT:
	case NN_PREINIT_WAITING_FOR_MATCHUP:
		// We got an ack, but we're not ready to start the matchup yet.
		break;
	case NN_PREINIT_READY:
		// We're ready to start the matchup now.
		SendInitPackets(neg);
		break;
	}
}
...
void NNProcessData(char *data, int len, SOCKADDR_IN *fromaddr)
{
	...
	else if (ptype == NN_PREINIT_ACK)
	{
		if (len < PREINITPACKET_SIZE)
			return;
		memcpy(&p, data, PREINITPACKET_SIZE);
		neg = FindNegotiatorForCookie((int)ntohl((unsigned int)p.cookie));
		if(neg)
			ProcessPreinitAckPacket(neg, &p);
	}
