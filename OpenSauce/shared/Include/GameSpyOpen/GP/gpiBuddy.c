								...
								GPIBool buddySync)
{
	...
	// Added a sync requested value that tells the backend to automatically add
	// me and my friend to each other's buddy lists. 
	// NOTE: a signature is required to do this!
	if (buddySync)
	{
...
#endif
		gpiBuddyDeletedLocally(connection, profileid,gsi_false);
    }
...
)
{
	...
	// Mark this buddy deleted in the buddy list.
	gpiBuddyDeletedLocally(connection,profile,gsi_true);
...
//we do not remove the buddy from the list anymore
// 		if(gpiCanFreeProfile(pProfile))
// 			gpiRemoveProfile(connection, pProfile);
...
{
	// This function modifies the profile.deletedLocally flag for tracking a 
	// list buddies deleted for a session.
	GPIProfile * pProfile;
