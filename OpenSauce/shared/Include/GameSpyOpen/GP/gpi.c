	...
	do
	{
		...
		while((next = strstr(iconnection->socketBuffer.buffer, "\\final\\")) != NULL)
		{
			...
			else
			{
                ...
                else if(strncmp(iconnection->inputBuffer, "\\rar\\", 5) == 0)
                {
                    // Process the remote auth response. This lets us know the
					// proper namespaceid and partnerid based on the authtoken
					// credentials.
                    CHECK_RESULT(gpiProcessRemoteAuthResponse(connection, iconnection->inputBuffer));
                }
				else if (strncmp(iconnection->inputBuffer, "\\addbuddyresponse\\", 18) == 0)
				{
					CHECK_RESULT(gpiProcessRecvAddBuddyResponse(connection, iconnection->inputBuffer));
				}
				else
				{
...
)
{
	...
	sprintf(buf, "%d profiles %d bytes (%d avg)", nProfiles, nTotalMemory, nTotalMemory / GS_MAX(nProfiles, 1));
	report(buf);
	if(nProfiles)
	{
		sprintf(buf, "UserID: %d (%d%%)", nUserID, nUserID * 100 / nProfiles);
		report(buf);
		sprintf(buf, "BuddyStatus: %d (%d%%) %d bytes (%d avg)", nBuddyStatus, nBuddyStatus * 100 / nProfiles, nBuddyMemory, nBuddyMemory / GS_MAX(nBuddyStatus, 1));
		report(buf);
		sprintf(buf, "InfoCache: %d (%d%%) %d bytes (%d avg)", nInfoCache, nInfoCache * 100 / nProfiles, nInfoMemory, nInfoMemory / GS_MAX(nInfoCache, 1));
		report(buf);
