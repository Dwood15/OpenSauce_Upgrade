...
// TableHashFn
// -----------
// TableHashFn is a pointer to a client-supplied function that the 
...
// TableCompareFn
// --------------
// TableCompareFn is a pointer to a client-supplied function that the 
...
typedef void (*TableElementFreeFn)(
	void 
	* elem
);
...
// would require knowledge of the elements' structure, which the HashTable 
...
// which knows what the elements are, can do the appropriate deallocation of 
// any embedded pointers through that function. 
...
// which bucket in which to place the new element. Its contents are copied 
// from the memory pointed to by newElem. If there is already an element in 
// the table that is determined to be equal (using the comparison function) 
// this will use the contents of the new element to replace the previous 
// element, calling the free function on the replaced element.
...
// TableRemove
// ----------
// Remove a element frin the table. If the element does not exist, the function 
// returns 0. If it exists, it returns 1 and calls the free function on the 
// removed element.
