		...
		for(fieldIndex = 0 ; fieldIndex < numFields ; fieldIndex++)
		{
			...
			else if(gsXmlMoveToChild(request->mSoapResponse, "unicodeStringValue"))
			{
				...
				if(value)
					...
					value = "";
#if defined(_UNIX) && defined (GSI_UNICODE)
				valueUnicode = UTF8ToUCSStringAlloc(value);
#else
			...
			{
				// Only in the case of a null field should we get an empty 
				// RecordValue in the SOAP response (and this case should only 
				// happen if an ATLAS 2.0 stat has no default set).
				outputField->mType = SAKEFieldType_NULL;
			}
		}
	}
