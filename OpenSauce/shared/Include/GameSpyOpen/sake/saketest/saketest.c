...
#else
	#define SAKE_UPLOAD_AMOUNT (128*1024) //smaller stack size
	#define SAKE_DOWNLOAD_AMOUNT (128*1024*2)
#endif
...
///////////////////////////////////////////////////////////////////////////////
// This is a big Validation function to ensure integrity of data received.
static void Validate(char * requestType, void *inputData, void *outputData)
{
	SAKEField ** records;
	SAKEField * field;
	...
	if (!results[gIndex].pass) // If this already failed, don't change the failure reason.
		return;
...
{	
	...
	// Here we think to process tasks.
...
// This prints request result, returns gsi_false for errors.
...
static void SearchForRecordsCallback(SAKE sake, SAKERequest request, SAKERequestResult result, void *inputData, void *outputData, void *userData)
{
	...
	HandleRequestResult(result,"SearchForRecords");
...
static void GetMyRecordsCallback(SAKE sake, SAKERequest request, SAKERequestResult result, void *inputData, void *outputData, void *userData)
{
	...
	// Now validate the data and ensure that it's ours.
...
// We shouldn't need this since we have upload content.
                                             ...
                                             void     *userData)
{
    ...
    else
    {
        ...
        {
            // We must have asked for a download to memory, so update the Sake 
			// image test data with it.
...
static void ReportRecord(SAKEReportRecordInput * input)
{
	...
	ProcessTasks();
	gIndex++; // Increment the global task counter.
}
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
// We shouldn't need this now that we have upload/download content.
...
static void UploadContent(SAKEContentInfo *contentInfo, gsi_char *remoteFilename)
{
    ...
    {
        // We must be uploading from disk, so check the local file name.
...
{
	// This is a FULL rundown of generic tests of all the functionality.
    ...
    {
			// This is for downloading to memory if NOFILE is defined.
			char * downloadBuffer;
...
int test_main(int argc, char* argv[])
{
	...
	// Shutdown the GameSpy Core SDK.
	printf("Shutting down the GameSpy Core\n");
	gsCoreShutdown();

    // Wait for GameSpy Core SDK shutdown (it should be instantaneous unless 
	// you have multiple SDK Cores).
