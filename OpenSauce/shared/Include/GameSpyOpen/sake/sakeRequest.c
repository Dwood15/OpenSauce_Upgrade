...
SAKERequest SAKE_CALL sakeiInitRequest(SAKE sake, SAKEIRequestType type, void *input, SAKERequestCallback callback, void *userData)
{
	...
	// Initialize the request init result to success.
                                          ...
                                          void           *param)
{
    ...
    {	
        // Check to see if we have a pointer to the developer's progress 
		// callback.
...
static GHTTPBool sakeiDownloadContentCompletedCallback(GHTTPRequest   request, 
                                                ...
                                                void           *param)
{
    GHTTPBool      freeBuffer = GHTTPTrue;	// When downloading memory and on error, this
											// callback tells the http to release the buffer.
                                                      ...
                                                      void *userData )
{
	...
	// Make sure to include session token (and other stuff) in headers if we 
	// have one set (needed for apigee verification).
	strcpy(headerString, "");
	...
	strcat(headerString, "\r\n");	// It seems that http requires a trailing carriage
									// return when no body is present.

	if(input->content->mType==SAKEContentStorageType_MEMORY)
	{   

#ifdef GSI_UNICODE
		AsciiToUCSString(headerString, uniHeaderString);
		request = ghttpGetEx(downloadUrl,	// The download URL.
			uniHeaderString,				// Any extra header info.
			input->content->mStorage.mMemory->mBuffer,  // Buffer to which to download.
                                      ...
                                      void           *param)
{
    ...
    if (state == GHTTPReceivingFile)
    {	
        // Check to see if we have a pointer to the developer's progress callback.
        ...
        else
        {
            ...
            else
            {
                ...
                if (result == gsi_true)
                {
					...
					// Make sure to include the session token (and other stuff) in headers if 
					// we have one set (needed for apigee verification).
					...
					strcat(headerString, "\r\n");	// It seems that http requires a trailing
													// carriage return before dime attachment.
