...
#define SAKEI_FILE_DOWNLOAD_PARAMS  GS_USTR _T("?fileid=%d&gameid=%d&pid=%d")
...
SAKEStartupResult SAKE_CALL sakeStartup(SAKE * sakePtr)
{
	...
	// Check that the GameSpy SDK Core is initialized.
	if(gsCoreIsShutdown())
...
void SAKE_CALL sakeSetGame(SAKE sake, const gsi_char * gameName, int gameId, const gsi_char *secretKey)
{
	...
	UCSToAsciiString(gameName, sake->mGameName);
	UCSToAsciiString(secretKey, sake->mSecretKey);
#else
...
gsi_bool SAKE_CALL sakeGetFileUploadURL(SAKE sake, gsi_char url[SAKE_MAX_URL_LENGTH])
{
    ...
    else
	{
		...
		AsciiToUCSString(sake->mGameName, uniGameName);
