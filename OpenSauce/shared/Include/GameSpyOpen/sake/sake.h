...
// Used in sakeUploadContent for check maximum content filename length.
#define SAKE_FILENAME_STR_MAX 64
...
// Parameters
//		sakePtr	: [in] Pointer to a Sake object, which is initialized by 
//						startup. This will be used in nearly all subsequent 
//						Sake calls.
// Returns
//		An enumeration of possible results.  If the result is
//		SAKEStartupResult_SUCCESS, then the startup has succeeded. Any other 
//		value indicates a failure, and the game should not continue calling 
//		other Sake functions.
// Remarks
//		Before using Sake, the GameSpy Availability Check must have been 
//		performed and indicated that the GameSpy services are available for 
//		this game title and the Core object must have been initialized by 
//		calling gsCoreInitialize. Sample code for this is available	in the Sake 
//		test app.
//		<p />
...
// Parameters
//		sake		: [in] The Sake object.
//		gameName	: [in] Your title's gamename, as assigned by GameSpy.
//		gameId		: [in] Your title's gameid, as assigned by GameSpy.
//		secretKey	: [in] Your title's secret key, as assigned by GameSpy.
// Remarks
//		The function provides no indication of whether or not the gamename 
//		and gameid are correct. If they are not correct, your subsequent 
//		Sake requests may fail.
//		<p />
//		Your game will also need to call sakeSetProfile to authenticate the  
//		current player before making Sake requests.
// See Also
//		sakeSetProfile
...
//							AuthService.
// Remarks
//		The profile id, cert, and proof are obtained from the AuthService 
//		callback (e.g., after calling wsLoginUnique).<p />
//		As with sakeSetGame, sakeSetProfile provides no indication of 
//		whether or not the information provided is correct. The Sake 
//		service checks these values and uses them to authenticate the  
//		player and, for certain requests, to identify which player's 
//		data is being accessed or updated.
// See Also
//		Authservice\WSLoginProfile (or WSLoginUnique, or WSLoginRemote), 
//		sakeSetGame
...
typedef enum{
	...
	SAKEFieldType_INT64,
	SAKEFieldType_NULL,  // This field is set to null; this should only happen for ATLAS stats with no default set.
...
//////////////////////////////////////////////////////////////////
// SAKEContentStorageType
// Summary
//		This enum is used to determine the type of local storage that the SDK 
//		will use to upload/update from or download to from the backend.
// Remarks
//		See SAKEContentInfo structure.
typedef enum SAKEContentStorageType{
...
typedef enum{
	...
	SAKERequestResult_NO_PERMISSION,			// The profile is not the owner of the record and the record does not 
												// have public access (permissions are set on the Sake Web Admin Panel).
	SAKERequestResult_RECORD_LIMIT_REACHED,		// The profile cannot create any more records in the table specified 
												// (Limit per owner is set on the Sake Web Admin Panel).
	...
	SAKERequestResult_NOT_RATEABLE,				// The record is in a table that is not rateable (The 'Rateable' flag
												// for a table is set on the Sake Web Admin Panel).
	...
	SAKERequestResult_INVALID_SESSIONTOKEN,     // Make sure wsSetGameCredentials was called with valid credentials and you 
												// have logged in via AuthService.
...
// SAKEContentBuffer
// Summary
//		Memory location and the length for the locally stored content.
// Members/Constants
//		mBuffer	: [in/out] Pointer to the memory location.
//		mLength	: [in/out] Size of buffer.
// Remarks
//		This used when the content is saved in a memory location.<p>
// See Also
//		SAKEContentStorage
typedef struct SAKEContentBuffer{
...
// Remarks
//		If local content is stored in memory, use  mMemory.
//      If local content is stored in a file, use  mFile.
typedef union SAKEContentStorage{
...
// SAKEContentInfo
// Summary
//		Structure to pass all the related information regarding content to the 
//		API calls. 
// Remarks
//		The mFileid refers to the file id stored by the service.
// See Also
//		SAKEContentStorageType, SAKEContentStorage, sakeUploadContent,
//		 sakeDownloadContent, sakeUpdateContent
typedef struct SAKEContentInfo{
...
//		outputData  : [in]	Pointer for the input data from API callback 
//							function.
//		userData	: [in/out]	The developer specified void pointer data 
//								shared between the API and its callback.
// Remarks
//		Not all request types have output objects. If a request type does not 
//		have an output object, then outputData will be always be NULL when 
...
/////////////////////////////////////////////////////////
// SAKEUploadContentProgressCallback
// Summary
//		This typedef defines the progress callback function for the upload 
//		content API. It is invoked periodically to indicate data transfer 
//		progress.
// Parameters
//      sake            : [in]	The Sake instance the callback is running on.
...
//									shared between the API and its callback. 
// See Also
//		SAKEUploadContentCompletedCallback
typedef void (*SAKEUploadContentProgressCallback)(
	SAKE      sake, 
...
// SAKEUploadContentCompletedCallback
// Summary
//		This typedef defines the callback function called when the Upload 
//		Content request completes. After this is called if successful, the data 
//		is uploaded to the back end and a new file id is assigned.
// Parameters
//      sake        : [in] The Sake instance the callback is running on.
//		fileId	    : [in] A unique new file id to point to the content at the 
//							server.
//		result	    : [in] The upload file result as it is returned by the 
//							service call.
//		userData    : [in/out] The developer specified void pointer data shared 
//                         between the API and its callback. 
//      IMPORTANT: If uploading from memory developer must delete the 
//		allocated memory when the buffer is no longer needed.
// Remarks
//		IMPORTANT: If uploading from memory developer must delete the 
//		allocated memory when the buffer is no longer needed.<p>
// See Also
//		SAKEFileResult, SAKEUpdateContentProgressCallback
typedef void (*SAKEUploadContentCompletedCallback)(
	SAKE				sake, 
	SAKERequestResult	result,
	gsi_i32				fileId, 
...
////////////////////////////////////////////////////////////
// SAKEDownloadContentProgressCallback
// Summary
//		This typedef defines the progress callback function for the download 
//		content API. It is invoked periodically to indicate data transfer 
//		progress.
// Parameters
//      sake            : [in] The Sake instance the callback is running on.
...
//									shared between the API and its callback. 
// See Also
//		SAKEDownloadContentCompletedCallback
typedef void (*SAKEDownloadContentProgressCallback)(
	SAKE    sake, 
    gsi_u32 bytesTransfered, 
    gsi_u32 totalSize, 
    void    *userData
...
// SAKEDownloadContentCompletedCallback
// Summary
//		This typedef defines the callback function called when the Download 
//		Content request completes. After this is called if successful, the 
//		content is downloaded to the specified memory location or to a file.
// Parameters
//      sake        : [in] The Sake instance the callback is running on.
//		result	    : [in] The download file result as it is returned by the 
//							service call.
//		buffer	    : [in] The buffer which contains the downloaded content, if 
//							downloading to memory. Otherwise, it is NULL.
//		bufferLength: [in] The buffer length for the downloaded content, if 
//							downloading to memory. Otherwise, it is 0.
//		userData    : [in/out] The developer specified void pointer data shared 
//								between the API and its callback. 
// Remarks
//      IMPORTANT: If downloading to memory, the developer must delete the 
//					allocated memory by the callback when the buffer is no
//					longer needed.
// See Also
//		SAKEFileResult, SAKEDownloadContentProgressCallback
typedef void (*SAKEDownloadContentCompletedCallback)(
	SAKE				sake, 
    SAKERequestResult	result,
    SAKEFileResult		fileResult, 
    char				*buffer, 
...
// sakeGetStartRequestResult
// Summary
//		Called to retrieve the result of a request. Normally used to determine 
//		the reason for a failed request.
// Parameters
//		sake	: [in] The Sake object
// Returns
//		Enum value used to indicate the specific result of the request.
// Remarks
//		This function will always return the most recent request that was 
//		attempted, so it must be called immediately after a failure to get the 
...
//		sakeReportRecord
typedef struct{
	char  *mTableId;	// Points to the tableid of the table in which the 
						// record to be reported exists.
	int    mRecordId;	// The recordid of the record to report.
	int    mReasonCode; // A code for the report reason, for if the developer 
						// has a few predefined options
	char  *mReason;		// Description of reasons why the record is reported. 
...
// sakeReportRecord
// Summary
//		Report a specified record to be moderated.
// Parameters
//		sake    	: [in] The Sake object.
//		input		: [in] Stores the data for the record you wish to report.
//		callback	: [in] The request callback function.
//		userdata	: [in] Pointer to user-specified data sent to the 
//							request callback.
// Returns
//		Reference to internal object that tracks the request. If this is NULL, 
//		then the request has failed to initialize. You can call 
//		sakeGetStartRequestResult to obtain the reason for the failure.
// Remarks
//      This may only be called for a table with the Content Moderation feature 
//		enabled. Contact your account manager if you are interested in Content 
//		Moderation.
// See Also
//		SAKEReportRecordInput, SAKERequestCallback
COMMON_API SAKERequest SAKE_CALL sakeReportRecord(
	SAKE					sake, 
	SAKEReportRecordInput	*input, 
...
// Remarks
//		Any fields which are not initialized will be set to their default 
//		value. If mNumFields is 0, indicating that no initial values will be 
...
typedef struct{
	...
	SAKEField *mFields;		// Points to an array of fields which has the 
							//	initial values for the new record's fields.
...
// Summary
//		Returned output object that specifies the recordid for the 
...
// sakeCreateRecord
// Summary
//		Creates a new Record in a Sake table.
// Parameters
//		sake		: [in] The Sake object.
...
//		sakeGetStartRequestResult to obtain the reason for the failure.
// Remarks
//		If the request completed successfully, then the output object contains 
//		the recordid of the newly created record.
// See Also
...
//		<p />
//		UpdateRecord does not have an output object, because the Sake service 
//		does not send any response other than the success or failure 
//		indicated by the result parameter passed to the callback. When the 
//		callback is called, the outputData parameter will always be set to 
//		NULL.
// See Also
//		sakeUpdateRecord
typedef struct{
	char      *mTableId;	// Points to the tableid of the table in which 
							// the record to be updated exists.
	int        mRecordId;	// Identifies the record to be updated.
	SAKEField *mFields;		// Points to an array of fields which has the new 
...
//		NULL, then the request has failed to initialize. You can call 
//		sakeGetStartRequestResult to obtain the reason for the failure.
// Remarks
//      UpdateRecord does not have an output object, because the Sake 
//		service response only indicates success or failure in the 
//		registered callback function. When the registered callback is 
//		invoked, the outputData parameter will always be set to NULL.
// See Also
//		SAKEUpdateRecordInput, SAKERequestCallback, sakeGetStartRequestResult
...
// Remarks
//		DeleteRecord does not have an output object, because the Sake
//		service response only indicates success or failure in the 
//		registered callback function. When the registered callback is 
//		invoked, the outputData parameter will always be set to NULL.
// See Also
...
//		"average_rating" fields (if the table has its ratings option set 
//		to true).
// See Also
...
typedef struct{
	...
	char	  **mFieldNames;			// Points to an array of strings, each of which contains the name of 
										...
										// that will be returned as part of the response. The array can 
	...
	int        mOffset;					// If not set to 0, then the Sake service will return records starting from the 
...
typedef struct{
	...
	SAKEField **mRecords;		// Points to an array of records, each of which is 
...
// Parameters
//		sake		: [in] The Sake object.
//		input		: [in] Stores the info about the records for which you wish 
//							to search.
//		callback	: [in] The request callback function.
//		userData	: [in] Pointer to user-specified data sent to the request 
//							callback.
// Returns
//		Reference to internal object that tracks the request. If this is 
//		NULL, then the request has failed to initialize. You can call 
...
//		contains contains records founds by the search. See the definitions 
//		of the Input & Output structs for more information about how to 
//		limit what is retrieved in the request and certain metadeta fields 
//		that can be retrieved.
// See Also
//		SAKESearchForRecordsInput, SAKESearchForRecordsOutput, 
//		SAKERequestCallback, sakeGetStartRequestResult
...
// Remarks
//		In addition to the fields which the developer defines, you can also 
//		request values for the "recordid" field, "ownerid" field (if the table 
//		has an owner-type of profile), and "num_ratings" and "average_rating" 
//		fields (if the table has its ratings option set to true).
// See Also
//		sakeGetMyRecords
typedef struct{
	...
	int        mNumFields;	// Stores the number of strings in the mFieldNames array. This list 
...
// Summary
//		Returned output object that specifies all of the records which the 
//		local player owns in the table.
// See Also
//		sakeGetMyRecords, SAKERequestCallback, SAKEField
typedef struct{
	int         mNumRecords;	// The number of records found.
	SAKEField **mRecords;		// Points an array of records, each of which is 
								// represented as an array of fields.
} SAKEGetMyRecordsOutput;
...
// Remarks
//		If the request completed successfully, then the output object 
//		contains all of the records which the local player owns in the 
//		table. See the definitions of the Input & Output structs for more 
//		information about how to limit what is retrieved in the request 
//		and certain metadeta fields that can be retrieved.
// See Also
...
// Remarks
//		In addition to the fields which the developer defines, you can also 
//		request values for the "recordid" field, "ownerid" field (if the 
//		table has an owner type of profile), and "num_ratings" and 
...
typedef struct{
	...
	char     **mFieldNames;		// Points to an array of strings, each of which contains the name 
...
// Summary
//		Returned output object that contains all of the records which were 
...
// Remarks
//		If the request completed successfully, then the output object contains 
//		all of the records which were specified in the request. See the 
//		definitions of the Input & Output structs for more information about 
//		how to limit what is retrieved in the request and certain metadeta 
...
// Remarks
//		If no record was found due to constrained search criteria, the returned 
//		record will be set to NULL.<p>
typedef struct{
...
//		sakeGetStartRequestResult to obtain the reason for the failure.
// Remarks
//		The output will always be a single record (unless no records pass the 
//		filter, in which case the output will contain NULL data for the 
//		returned record). Note that this function works best in a table in 
//		which records are not deleted or are deleted in	order of oldest first 
//		(in other words, in tables in which recordids are contiguous).
// See Also
//		SAKEGetRandomRecordInput, SAKEGetRandomRecordOutput, 
//		SAKERequestCallback, sakeGetStartRequestResult
...
// Remarks
//		The range of ratings which Sake supports is 0 to 255. However, a game 
//		can restrict itself to a subset of that range if it wishes. 
//		<p />
//		For example, a game may want to use a rating scale of 1 to 	5 (e.g., 
//		stars), or it may want to use a range of 0 to 100. Sake allows users to 
//		rate records which they own, however no player profile can rate a 
//		single record more than once. 
//		<p />
//		Internally, Sake stores every rating given by every user, which allows 
//		it to compute accurate averages and to prevent repeat ratings. The 
//		field name "my_rating" can be used to obtain the current profile's rating 
//		for a given record or when searching for records. By default, 
//		my_rating = -1 for records the current user has not yet rated. When 
//		browsing for records, you can use the special search tags @rated or 
//		@unrated in the filter string to limit the searches to either rated or 
//		unrated records.
// See Also
//		sakeRateRecord
typedef struct{
...
// Summary
//		Returned output object that lists the new number of ratings and the new 
//		average rating for the specified record.	
// See Also
...
//		sakeGetStartRequestResult to obtain the reason for the failure.
// Remarks
//		The range of ratings which Sake supports is 0 to 255. However, a 
//		game can restrict itself to a subset of that range. For example, 
//		a game may want to use a rating of 1 to 5 (a star rating), or it 
//		may want to use a range of 0 to 100.
//		<p />
//		Sake allows users to rate records which they own, however no profile 
//		can rate the same record more than once.
// See Also
...
// Summary
//		Returned output object that specifies the maximum number of records 
//		that a profile can own in the table.
// See Also
//		sakeGetRecordLimit, SAKERequestCallback
typedef struct{
	int mLimitPerOwner;	// Contains the maximum number of records that a profile 
						// can own in the table; corresponds to the limit per owner 
						// option that can be set using the Sake Web Admin Panel.
	int mNumOwned;		// Contains the number of records that the local profile 
...
// Remarks
//		If the request completed successfully, then the Output object 
//		contains info about the record limit for the specified table.
// See Also
//		SAKEGetRecordLimitInput, SAKEGetRecordLimitOutput, 
//		SAKERequestCallback, sakeGetStartRequestResult
...
typedef struct{
	char	  *mTableId;	// Points to the tableid of the table to be searched.
	gsi_char  *mFilter;		// SQL-like filter string which is used to filter which records 
							// are to be looked at when getting the record count.
	gsi_bool   mCacheFlag;	// Enables caching if set to gsi_true. Defaults to no caching 
...
typedef struct{
	int	       mCount;	// Contains the value of the record count. If no records 
...
// Parameters
//		sake		: [in] The Sake object.
//		input		: [in] Stores info on the table whose count you wish to 
//							check.
//		callback	: [in] The request callback function.
//		userData	: [in] Pointer to user-specified data sent to the 
//							request callback.
...
// Remarks
//		If the request completed successfully, then the Output object
//		contains info about the count for the specified table.
//		<p>
//		This function should really be called very seldomly. Example: after the 
//		first call for a specific leaderboard, the information this function 
//		returns will be the same, so it should be cached for subsequent page 
//		calls.
// See Also
...
// sakeGetFieldByName
// Summary
//		This utility function retrieves the specified field from the record, 
//		via the name that identifies it.
// Parameters
...
// Returns
//		Pointer to a SAKEField which represents the field that was identified 
//		by the given name.
// See Also
//		SAKEField
...
// sakeCancelRequest
// Summary
//		Used for cancelling any outstanding requests made to the Sake Storage 
//		Server.
// Parameters
//		sake		: [in] The Sake object.
//		request		: [in] The returned value from the original request to be 
//							cancelled.  
// Returns
//		SAKEStartRequestResult_SUCCESS if successful.
// Remarks
//		To be used for cancelling requests which taking too long or 
//		unresponsive. No actions are cancelled at the backend if the request 
//		was processed and response was not received yet. The cancel request 
//		only to prevent the SDK waiting indefinitely for a response from the 
//		backend.
...
// Summary
//		Override the default Sake file download URL. Do not use.
COMMON_API gsi_bool SAKE_CALL sakeSetFileDownloadURL(
	SAKE		sake, 
...
// sakeSetFileDownloadURL
// Summary
//		Override the default Sake file upload URL. Do not use.
COMMON_API gsi_bool SAKE_CALL sakeSetFileUploadURL(
	SAKE		sake, 
	gsi_char	url[SAKE_MAX_URL_LENGTH]
);
//DOM-IGNORE-END
...
// Parameters
//		sake		: [in] The Sake object. 
//		fileid		: [in] The file id returned by the headers of the uploaded file. 
//							Call sakeGetFileIdFromHeaders to obtain this.
...
//////////////////////////////////////////////////////////////////
// SAKEUploadContentInput
typedef struct SAKEUploadContentInput{
    gsi_char        *remoteFileName;            // file name to saved be the server
    SAKEContentInfo *content;                   // pointer to the content info structure
    gsi_bool        transferBlocking;           // gsi_true if request is a blocking call
    SAKEUploadContentProgressCallback  progressCallback; 
} SAKEUploadContentInput;

//////////////////////////////////////////////////////////////
// sakeUploadContent
// Summary
//		This API  function allows uploading content either 
//		from a memory location or from a file to the server.
// Parameters
//		sake             : [in]  The instance of the SDK.
//		input            : [in]  The pointer to the structure which contains the information 
//									for the local content to be uploaded.
//      completedCallback: [in]  Developer's callback function when
//									the upload completes.
//		userData         : [in/out] Optional. The developer specified void pointer data
//									shared between the API and its callback. Set to NULL if unused. 
// Returns
//		SAKEStartRequestResult_SUCCESS if successful.
// Remarks
//      This API call should be invoked only when a new file 
//		for the content is being created at the back end.
...
//////////////////////////////////////////////////////////////////
// SAKEDownloadContentInput
typedef struct SAKEDownloadContentInput
{
    SAKEContentInfo  *content;      // Pointer to content info structure.
    gsi_bool transferBlocking;      // gsi_true if request is a blocking call.
    SAKEDownloadContentProgressCallback  progressCallback; // Developer's callback to track download progress.
} SAKEDownloadContentInput;

//////////////////////////////////////////////////////////////
// sakeDownloadContent
// Summary
//		This API function allows downloading content from the backend service
//      either to a memory location or to a local file. 
// Parameters
//		sake             : [in]  The instance of the SDK.
//		input            : [in]  The pointer to the structure which contains the information 
//								 for the local content to be downloaded. 
//		progressCallback : [in]  Optional. Developer's callback function to 
//								 keep track of the downloading. Set to NULL if unused. 
//      completedCallback: [in]  Developer's callback function 
...
//		SAKEStartRequestResult_SUCCESS if successful.
// Remarks
//      When downloading to memory, there is no need to preallocate 
//		memory for the buffer prior to calling 
//      this function. The buffer pointer and the buffer length, 
//      are updated with the memory location and the size by the SDK.
//      For downloading to a file, the file path must exist 
...
// Parameters
//		headers		: [in] The headers to parse for the file id. You can get 
//							these from ghttpGetHeaders. 
//		result		: [ref] Reference to the result as obtained in the headers. 
// Returns
//		gsi_true if it was able to parse the result successfully, gsi_false 
//		otherwise.
// Remarks
//      You can also check the headers manually for the Sake-File-ResultÅE
//		Eheader. The value stored in the header is an integer, the possible 
//		values of which are enumerated in SAKEFileResult. SAKEFileResult_SUCCESS 
//		means that the file was uploaded successfully, while any other value 
//		indicates that there was an error uploading the file.
// See Also
//		SAKEFileResult
...
// sakeGetFileIdFromHeaders
// Summary
//		If the file was uploaded successfully, this function obtains the file 
//		id that references the file.
// Parameters
//		headers		: [in] The headers to parse for the file id. You can get 
//							these by calling ghttpGetHeaders. 
//		fileId		: [ref] The file id returned by SakeFileServer when the 
//							file was uploaded. 
// Returns
//		gsi_true if able to parse the file id successfully, gsi_false otherwise.
// Remarks
//      To get the file id from the headers manually, look for the Sake-File-Id 
//		Eheader. Once obtained, the file id can now be stored in a file id 
//		field in the database. 
//		<p />
//		Note: If you upload a file, but do not store the file id in the database, 
//		then the Sake service may automatically delete the file after 
...
// sakeThink
// Summary
//		This function is used for processing the ghttp request and 
//		responses. It should be called in the main loop.
