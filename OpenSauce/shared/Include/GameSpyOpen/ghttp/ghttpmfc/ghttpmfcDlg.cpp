...
)
{
	...
	// Added new states for asynch DNS lookup - set all to HOST_LOOKUP for 
	// radio buttons.
	...
	if(SUCCEEDED(result))
	{
		...
		{		
			...
			if(SUCCEEDED(result) && (len > 0))
			{
				...
				// Apparently, ports are optional - I would presume that you 
				// should revert to the default port when it is missing.
				//
				...
				{
					nStart = -1;
					nEnd = 0; // This won't be used; we need to initialize it to prevent compiler warning.
				}
