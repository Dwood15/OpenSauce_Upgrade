...
#ifdef __MWERKS__  // Prevent strict compiler warnings.
...
)
{
	...
	// Don't free this buffer, it's from the stack.
	if(index == 1)
		return GHTTPFalse;
...
)
{
	...
	{
		// Display based on whether or not we know the total size.
		if(totalSize != -1)
