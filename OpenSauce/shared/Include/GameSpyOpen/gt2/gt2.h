...
//		outgoingBufferSize	: [in] The byte size of the per-connection buffer
//									 for reliable outgoing messages.  
//									Can be 0 to use the internal default.
//		incomingBufferSize	: [in] The byte size of the per-connection buffer
//									 for out-of-order reliable incoming 
//									 messages.
//									Can be 0 to use the internal default.
...
//		 application to communicate with other applications (through their own
//		 sockets) that are typically on remote machines, although they can also
//		 be on the local machine (the other application will often be referred
//		 to as the "remote machine", even though technically it may be the same
//		 machine).
//		A single socket allows an application to both accept connections from
//		 remote machines and make connections to remote machines. For most
//		 applications, only one socket needs to be created.	All incoming
//		 connections can be accepted on the socket, and all outgoing connections
//		 can be made using the socket.
//		A socket is created with the gt2CreateSocket function. If the function
//		 returns GT2Success then the socket was successfully created and bound
//		 to the local address (if one was provided). The socket that the
//		 "socket" parameter points to is valid until it is closed with
//		 gt2CloseSocket, or an error is reported to the gt2SocketErrorCallback
//		 callback parameter. It is now ready to be used for making outgoing
//		 connections, and can be readied for allowing incoming connections by
//		 calling gt2Listen.	If the return result is anything other than
//		 GT2Success, GT2 was unable to create the socket.<p>
// See Also
...
// If either buffer sizes is set to 0, a default value will be used (currently
// 4K).
// The buffer needs to be able to hold all messages waiting for confirmation
//  of delivery, and it needs to hold any messages that arrive out of order.
//  If either buffer runs out of space the connection will be dropped.
COMMON_API GT2Result gt2CreateVDPSocket
(
GT2Socket * socket,  // If the result is GT2Success, the socket object handle will be stored at this address.
const char * localAddress,  // The local address to bind to.
int outgoingBufferSize,  // Size of per-connection buffer where sent messages waiting to be confirmed are held, use 0 for default.
...
//		 closed callbacks will be called from within this function.<p>
// See Also
//		gt2CreateSocket
COMMON_API void gt2CloseSocket(GT2Socket socket);
...
//		 they can also be called from other places). It is possible that during
//		 this think the socket or any of its connections may be closed, so you
//		 must take care if calling other GT2 functions immediately after
//		 thinking. The more frequently this function is called, the faster GT2
//		 will be able to respond (and reply to) messages. The general rule is
//		 to call it at frequently as you can, although calling it faster than
//		 every 10-20 milliseconds is probably unnecessary. If you are using
//		 gt2Ping to measure ping times, then the accuracy of the latency
//		 measurement will increase with the frequency at which this function
//		 is called.<p>
COMMON_API void gt2Think(GT2Socket socket);
...
// To broadcast a datagram, omit the IP from the remoteAddress
//  (e.g., ":12345").
...
// Summary
//		This callback is called when a connection attempt with gt2Connect
//		finishes.
// Parameters
//		connection	: [in] The handle to the connection.
//		result		: [in] The result of the connection attempt. Anything aside
//							 from GT2Success indicates failure.
//		message		: [in] If result is GT2Rejected, this is the rejection
//							 message. May be NULL.
//		len			: [in] If result is GT2Rejected, the length of message. May
//							 be 0.
// Remarks
//		If result is GT2Success, then this connection attempt succeeded. The
//		 connection object can now be used for sending/receiving messages. Any
//		 other result indicates connection failure, and the connection object
//		 cannot be used again after this callback returns.
...
// gt2ReceivedCallback
// Summary
//		This callback is called when a message is sent from the remote system
//		 with a gt2Send.
// Parameters
//		connection	: [in] The handle to the connection.
...
//		 reliably, then it will always be received with this callback.
//		If it is not sent reliably, then the message might not be received, it
//		 might be received out of order, or it might be received more than once
//		 (very rare).<p>
// See Also
//		gt2Send
...
//		 gt2CloseConnection (or gt2CloseConnectionHard), either side closing
//		 the socket, or some sort of error.	The connection cannot be used again
//		 once this callback returns.<p>
// See Also
...
//		 make a round-trip from one connection to the other.
...
// See Also
//		gt2PingCallback
COMMON_API void gt2Ping(GT2Connection connection);
...
COMMON_API void gt2CloseConnection(GT2Connection connection);
...
COMMON_API void gt2CloseConnectionHard(GT2Connection connection);
...
COMMON_API void gt2CloseAllConnections(GT2Socket socket);
...
COMMON_API void gt2CloseAllConnectionsHard(GT2Socket socket);
...
COMMON_API void gt2Listen(GT2Socket socket, gt2ConnectAttemptCallback callback);
...
COMMON_API GT2Bool gt2Accept(GT2Connection connection, GT2ConnectionCallbacks * callbacks);
...
COMMON_API void gt2Reject(GT2Connection connection, const GT2Byte * message, int len);
...
COMMON_API GT2MessageID gt2GetLastSentMessageID(GT2Connection connection);
...
// See Also
//		gt2GetLastSentMessageID
COMMON_API GT2Bool gt2WasMessageIDConfirmed(GT2Connection connection, GT2MessageID messageID);
...
COMMON_API SOCKET gt2GetSocketSOCKET(GT2Socket socket);
...
COMMON_API void gt2SetUnrecognizedMessageCallback(GT2Socket socket, gt2UnrecognizedMessageCallback callback);
...
COMMON_API GT2Socket gt2GetConnectionSocket(GT2Connection connection);
...
COMMON_API GT2ConnectionState gt2GetConnectionState(GT2Connection connection);
...
COMMON_API unsigned int gt2GetRemoteIP(GT2Connection connection);
...
COMMON_API unsigned short gt2GetRemotePort(GT2Connection connection);
...
COMMON_API unsigned int gt2GetLocalIP(GT2Socket socket);
...
COMMON_API unsigned short gt2GetLocalPort(GT2Socket socket);
...
COMMON_API int gt2GetIncomingBufferSize(GT2Connection connection);
...
COMMON_API int gt2GetIncomingBufferFreeSpace(GT2Connection connection);
...
COMMON_API int gt2GetOutgoingBufferSize(GT2Connection connection);
...
COMMON_API int gt2GetOutgoingBufferFreeSpace(GT2Connection connection);
...
COMMON_API void gt2SetSocketData(GT2Socket socket, void * data);
...
COMMON_API void * gt2GetSocketData(GT2Socket socket);
...
COMMON_API void gt2SetConnectionData(GT2Connection connection, void * data);
...
COMMON_API void * gt2GetConnectionData(GT2Connection connection);
...
COMMON_API unsigned int gt2NetworkToHostInt(unsigned int i);
...
COMMON_API unsigned int gt2HostToNetworkInt(unsigned int i);
...
COMMON_API unsigned short gt2HostToNetworkShort(unsigned short s);
...
COMMON_API unsigned short gt2NetworkToHostShort(unsigned short s);
...
COMMON_API const char * gt2IPToHostInfo(unsigned int ip, char *** aliases, unsigned int *** ips);
...
COMMON_API const char * gt2StringToHostInfo(const char * string, char *** aliases, unsigned int *** ips);
...
// See Also
//		gt2IPToHostInfo
COMMON_API const char * gt2IPToHostname(unsigned int ip);
...
// See Also
//		gt2StringToHostInfo
COMMON_API const char * gt2StringToHostname(const char * string);
...
// See Also
//		gt2IPToHostInfo
COMMON_API char ** gt2IPToAliases(unsigned int ip);
...
// See Also
//		gt2StringToHostInfo
COMMON_API char ** gt2StringToAliases(const char * string);
...
// See Also
//		gt2IPToHostInfo
COMMON_API unsigned int ** gt2IPToIPs(unsigned int ip);
...
// See Also
//		gt2StringToHostInfo
COMMON_API unsigned int ** gt2StringToIPs(const char * string);
...
COMMON_API void gt2SetSendDump(GT2Socket socket, gt2DumpCallback callback);
...
COMMON_API void gt2SetReceiveDump(GT2Socket socket, gt2DumpCallback callback);
