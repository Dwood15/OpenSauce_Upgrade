...
GT2Socket _socket = NULL;

// NatNeg global variables
...
static GT2Bool OnUnrecognizedMessageCallback(GT2Socket socket, unsigned int ip, unsigned short port, GT2Byte * message, int len)
{
	...
	if (len >= NATNEG_MAGIC_LEN && memcmp(message, NNMagicData, NATNEG_MAGIC_LEN) == 0)
	{
		NNProcessData((char*)message, len, &saddr);
		return GT2True; // Packet handled by NNProcessData, return true.
	}

	return GT2False; // Packet must be Transport, return false.
}
...
int test_main(int argc, char **argv)
{
	...
	int msgloopcount = 0;
	GT2Result result;
	GSIACResult availresult;
	int i;
	NegotiateError error;

#if defined _PS3 || defined _PSP2
	
	// Add the appropriate parameters here for Sony devices (-c or -l), one on each side
	argc = 2;
	argv[1] = "-c";
	...
	while((availresult = GSIAvailableCheckThink()) == GSIACWaiting)
		msleep(5);
	if(availresult != GSIACAvailable)
	{
...
#ifdef _NITRO
	{
		...
		strcpy(remoteAddress, "10.129.136.165:12345");
	...
	{
		// create the socket
		result = gt2CreateSocket(&_socket, gt2AddressToString(0, 56666, NULL), 0, 0, SocketErrorCallback);
		if(result != GT2Success)
		{
		...
		gt2SetSendDump(_socket, SendDumpCallback);
		gt2SetReceiveDump(_socket, ReceiveDumpCallback);

		// set UnrecognizedMessageCallback
		gt2SetUnrecognizedMessageCallback(_socket, OnUnrecognizedMessageCallback);

		// listen if we're the server
		if(server)
		{
			gt2Listen(_socket, ConnectAttemptCallback);

			error = NNBeginNegotiationWithSocket(gt2GetSocketSOCKET(_socket), 777, 0, pc, cc, NULL);
			...
			printf("Listening for incoming connections [%s]...\n", gt2AddressToString(gt2GetLocalIP(_socket), gt2GetLocalPort(_socket), NULL));
		...
		error = NNBeginNegotiationWithSocket(gt2GetSocketSOCKET(_socket), 777, 1, pc, cc, NULL);
		...
		// gt2Connect requires remote address in following string format: "<IP address>:<Port>"
		strcpy(remoteAddress, inet_ntoa(otheraddr.sin_addr));
		sprintf(remoteAddress, "%s:%d", remoteAddress, ntohs(otheraddr.sin_port));
		
		printf("Attempting to connect to %s (max 10 sec)...\n", remoteAddress);
		result = gt2Connect(_socket, &Connection, remoteAddress, (GT2Byte *)initialMessage.data, initialMessage.len, 100000, &connectionCallbacks, GT2True);
		if(result != GT2Success)
		{
