								...
								void *           userData)
{
	...
	// Prepare the report hash.
	{
		...
		GSMD5_CTX md5;
		// Clear out the checksum portion of the header so that the MD5 hash is 
		// calculated on the entire report without a checksum.
								       ...
								       const gsi_u8  theAuthHash[16])
{
	...
	// Store the team ID here and use this in order to index the team game 
	// results.
	...
	{	
		...
		// If there are no problems, store the teamId.
...
{
	// Hardcoded to a 256-bit key block, although some ciphers will not use the 
	// entire block.

	// What to use for a random seed?
	//     1) 16 bytes from our Util_Rand seeded with current time.
	//     2) Hashed with 16 bytes from the cert public key (for a little extra 
	//		  randomness).
...
SCResult SC_CALL scPeerCipherEncryptBufferIV(SCPeerCipher * theCipher, gsi_u32 theMessageNum, gsi_u8 * theData, gsi_u32 theLen)
{
	...
	// NOTE: Using the same key for all messages would be extremely unsafe.
	GSMD5Init(&md5);
...
SCResult SC_CALL scCreateQueryParameterList(SCQueryParameterListPtr *queryParams, gsi_u32 queryParamsCount)
{
	...
	// We only accept valid SCQueryParameterList ** pointers, otherwise the 
...
SCResult SC_CALL scDestroyQueryParameterList(SCQueryParameterListPtr *queryParams)
{
	...
	// This pointer points to the same thing as ptrList. We just need to clear 
	// the dereferenced queryParams.
	*queryParams = NULL;
	return SCResult_NO_ERROR;
}
