...
// NOTE: broke up entity index in two; for later versions we will want to send 
// the 2 byte team index followed by the 4 byte team ID later in the stream.
...
typedef struct SCIReportHeader
{
	...
	gsi_u16  mPlayerCount;          // Players in the session.
	gsi_u16  mTeamCount;            // Teams in the session.
...
typedef struct SCIReport
{
	...
	//gsi_u32 mNumPlayersReported;	// This is to check against expected count - currently not used.
	gsi_u32 mNumResultsReported;	// This is to check against expected count.

	gsi_u32 mNumTeamsReported;	 // This keeps track of the number of team IDs that have been reported.
	gsi_u32 mTeamIds[SC_MAX_NUM_TEAMS];		// This is the internal list of team IDs.
...
// Call this when finished writing.
