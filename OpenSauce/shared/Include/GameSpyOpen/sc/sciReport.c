...
SCResult SC_CALL sciReportSetPlayerConnectionId(SCIReport * theReport, gsi_u32 thePlayerIndex, const gsi_u8 theConnectionId[SC_CONNECTION_GUID_SIZE])
{
	...
	// The roster section is just after the header.
...
SCResult SC_CALL sciReportSetPlayerGameResult(SCIReport * theReport, gsi_u32 thePlayerIndex, SCGameResult theGameResult)
{
	...
	// The results section is just after the auth section.
...
SCResult SC_CALL sciReportSetPlayerAuthInfo(SCIReport * theReport, gsi_u32 thePlayerIndex, const GSLoginCertificate * certificate, const gsi_u8 theAuthHash[16])
{
	...
	// The auth section is just after the roster.
	...
	// Copy the auth data into the buffer.
...
SCResult SC_CALL sciReportSetTeamGameResult(SCIReport * theReport, gsi_u32 theTeamIndex, SCGameResult theGameResult)
{
	...
	// Results section is just after the auth section. Team results are just 
	// after player results.
	resultsDataOffset = sizeof(SCIReportHeader) + aHeader->mRosterSectionLength + aHeader->mAuthSectionLength;
...
SCResult SC_CALL sciReportEnd(SCIReport * theReport, gsi_bool isAuth, SCGameStatus theStatus)
{
	...
	// Send the header in network byte order.
									  ...
									  gsi_i32     theValue)
{
	...
	// Calculate the length of data to be written.
	...
	// Update the count and length markers.
									    ...
									    gsi_i16     theValue)
{
	...
	// Calculate the length of the data to be written.
	writtenLen += sizeof(gsi_u16) + sizeof(gsi_u16); // 2 byte key ID, 2 byte key type;
									    ...
									    float       theValue)
{
	...
	// This needs real error handling code or an assertion on a non-constant 
	// expression.
	//else 
										 ...
										 const gsi_char * theValue)
{
	...
	// For unicode we need to store the ascii-converted string first in the 
	// buffer so as not to dynamically allocate space for it. Then once we have 
	// the length of it, we can write over the buffer.

#if defined(GSI_UNICODE)
	// First check the room in the buffer.
	GS_ASSERT(((theReport->mBuffer.mPos + (_tcslen(theValue)) + (sizeof(gsi_u16)*2))
		< theReport->mBuffer.mCapacity));

	// As long as we have enough room, add to the buffer and store length of 
	// the Ascii string. We'll need to remove 1 for the appended null char \0 
	// that we want to remove.
	asciiLen = UCSToAsciiString(theValue, &theReport->mBuffer.mData[theReport->mBuffer.mPos])-1;
	...
	numLenBytes = SevenBitEncoderWriteInt(NULL, (int)strlen(theValue)); // This finds out the number of bytes necessary to represent len.
	...
	// Prior to writing the string we need to prepend the length of the string 
	// to follow the .NET format of (string length)(String).
	SevenBitEncoderWriteInt(&theReport->mBuffer.mData[theReport->mBuffer.mPos], asciiLen);
	theReport->mBuffer.mPos += numLenBytes;

#if defined(GSI_UNICODE)
	// Now strip to Ascii and write the length - subtract 1 to get rid of 
	// appended null character.
	theReport->mBuffer.mPos += UCSToAsciiString(theValue, &theReport->mBuffer.mData[theReport->mBuffer.mPos])-1;
#else
