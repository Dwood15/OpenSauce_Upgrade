...
void SetupGameData(GameData &currentGame)
{
	...
	currentGame.m_GameName = (gsi_char *) _T("atlasSamples");
	...
	currentGame.m_Email = (gsi_char *) _T("sctest@gamespy.com");
	currentGame.m_Profile = (gsi_char *) _T("sctest01");
	currentGame.m_Password = (gsi_char *) _T("gspy");
	...
	// either ATLAS_KEY_USED_VEHICLE_1_P or ATLAS_KEY_USED_VEHICLE_2_P
	// The same goes for the track used key but with the keys related to the track.
	currentGame.m_PlayerKeys.push_back(new AtlasInt64Key(ATLAS_KEY_RACE_TIME_P, myRaceTime));
	currentGame.m_PlayerKeys.push_back(new AtlasByteKey(ATLAS_KEY_USED_VEHICLE_1_P, 1));
	...
	currentGame.m_GameKeys.push_back(new AtlasByteKey(ATLAS_KEY_USED_VEHICLE_1_G, 1));
}
...
int RunThroughSampleReport(std::vector<std::string> &args)
{
	...
	GSIStartAvailableCheck(myGameData.m_GameName);
