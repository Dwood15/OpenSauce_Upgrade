...
static void sciWsSubmitReportCustom(GHTTPPost thePost, void* userData)
{
	...
	// Use the internal method to get around unicode calls.
...
#else
	*message = UTF8ToUCSStringAllocLen(tempStr, tempStrLen);
#endif
...
#else
	stat->mName = UTF8ToUCSStringAllocLen(name, nameLen);
	stat->mValue = UTF8ToUCSStringAllocLen(val, valLen);
#endif
...
#else
	category->mName = UTF8ToUCSStringAllocLen(tempStr, tempStrLen);
#endif
...
{
	// We are parsing a single SCPlayerStatsQueryResponse, setting *response 
	// to the parsedResponse if everything parses correctly.
	gsi_u32 i;
	SCResult result;
...
{
	// We are parsing a single SCPlayerStatsQueryResponse, setting *response to 
	// the parsedResponse if everything parses correctly.
	gsi_u32 i;
	SCResult result;
								   ...
								   void*        userData)
{
	...
	if (httpResult == GHTTPSuccess)
	{
		if (gsiCoreGetAuthErrorCode() == GSAuthErrorCode_None)
		{
			// Success! We can now read header information to get our result 
			// code, and debug text message.
...
#ifdef GSI_UNICODE
			AsciiToUCSString(goastrdup(authError), textMessage);
#else
	...
	// We've checked this as part of the request and the webServices object 
	// below belongs to us, so there's no need to check callback ptr.
								  ...
								  void *userData)
{
	...
	// Check for pending request. We want to make sure no two asynchronous web 
	// service calls of the same type are executing at the same time.  
