...
static void PlayFakeMatch()
{
	...
	LOGI("  Playing our fake match so that we'll have some match data to report to ATLAS\n");
						   ...
						   void * userData)
{
	...
	{
		LOGI("    Success - report has been submitted\n");
		*reportSubmitted = 1;
	}
...
static gsi_bool SampleReportStats()
{
	...
	for (i = 0; i < NUM_PLAYERS; i++)
	{
		...
		LOGI("  Submitting the ATLAS report for player %d...\n", i+1);
...
{	
	...
	LOGI("Reporting stats for our sample match\n");
