...
// SCResult
// Summary
//		SCResult is used to indicate error and failure conditions in the 
//		ATLAS SDK.
// Remarks
//		Results of a call to an interface function or operation.  
//		It can be used to see if the initial call to a function completed 
//		without error. The callback that is passed to interface functions will 
//		also have a value that is of this type. The application can check this 
//		value for failures.
typedef enum{
	...
	SCResult_QUERY_TYPE_MISMATCH,	// Error occurs if the query id passed is used for the wrong query type (e.g., 
									// query id is passed into a game stats query instead of player stats query.
...
// SCGameResult
// Summary
//		Used when submitting a report for a game session to indicate the 
//		result of the game session for the current player or team.
// Remarks
...
//		report.
// Remarks
//		For <strong>SCGameStatus</strong> reporting, the game should do the 
//		following:
//		<p />
//		As long as the game finished properly, and no one disconnected during 
//		the course of play, then all players in the game session should submit 
//		<strong>SCGameStatus_COMPLETE</strong> reports. If any members 
//		disconnected during play, but the game was finished completely, then 
//		all players in the session should submit 
//		<strong>SCGameStatus_PARTIAL</strong> reports indicating that 
//		disconnects occured. For any players who do not complete the session, a 
//		<strong>SCGameStatus_BROKEN</strong> report should be submitted. 
//		<p />
//		Thus, if the game did not completely finish, all players will submit 
//		broken reports. The only case that will trigger an invalid report is if 
//		reports for the same game describe status as both 
//		<strong>SCGameStatus_COMPLETE</strong> and 
//		<strong>SCGameStatus_PARTIAL</strong>. 
//		<p />
//		Since COMPLETE indicates that all players finished the game without a 
//		disconnect and PARTIAL indicates that disconnects occured, at no time 
//		should a game report both COMPLETE and PARTIAL -- this will be seen as 
//		an exploit and invalidate the report.
// Members/Constants
...
// Remarks
//		Stores the parameters so that the query functions can serialize them. 
//		This must be created and filled before passing it to a query function. 
//		The user is responsible for destroying this object. <p>
// See Also
...
// SCStat
// Summary
//		User-facing object that contains a stat name, value, and type. The name 
//		and type of the stat correlates to that on the backend administration 
//		site. The type will be one of the values defined in the 
//		SCStatDataType.<p>
// See Also
...
// SCPlayer
// Summary
//		User-facing object that contains player data. Each player will have a 
//		profile id and some stats. <p>
// See Also
...
// SCTeam
// Summary
//		User-facing object that contains team data. Each team will have a 
//		team id and some stats. <p>
// See Also
...
// SCGameStatCategory
// Summary
//		Category object that contains a name and global game stats.	These 
//		objects will be a part of a SCGameStatsQueryResponse object that will 
//		be created by the SDK and passed to the user defined callback.<p>
// See Also
...
// Summary
//		Category object that contains a name and a set of players. These 
//		objects will be a part of a SCPlayerStatsQueryResponse object which 
//		will be created by the SDK and passed to the user defined 
//		callback.<p>
// See Also
...
// SCTeamStatsCategory
// Summary
//		Category object that contains a name and a set of teams. These objects 
//		will be a part of a SCTeamStatsQueryResponse object which will be 
//		created by the SDK and passed to the user defined callback.<p>
// See Also
...
// Summary
//		Response received from backend for game stats query. This object is 
//		comprised of stats for the requested category or categories.
// See Also
...
// Summary
//		Response received from backend for player stats query. This object is 
//		comprised of stats for the requested category or categories for each 
//		player in the return set.
// See Also
...
// Summary
//		Response received from backend for team stats query. This object is 
//		comprised of stats for the requested category or categories for each 
//		team in the return set.
// See Also
...
// Parameters
//		theInterface			: [in] The pointer to the SC Interface object. 
//										The game usually also has a copy of 
//										this.
//		httpResult  			: [in] Http result from creating a session.
//		result 					: [in] SC Result telling the application what 
//										happened when checking the game's ban 
//										list.
...
// See Also
//		
typedef void (*SCCheckBanListCallback)(
...
// Parameters
//		theInterface	: [in] A pointer to the ATLAS SDK object. The game 
//								usually also has a copy of this.
...
//		the result will be set to the specific error code. Otherwise, the 
//		result will be set to SCResult_NO_ERROR. See SCResult for error codes.
// See Also
...
//		userData		: [in] A constant pointer to user-provided data.
// Remarks
//		Called when a host or client reporting its intention is complete. The 
//		results will determine if the game session was successfully created. If 
//		there were any errors, the result will be set to the specific error code. 
//		Otherwise, the result will be set to SCResult_NO_ERROR.
//		<p />
...
//		that will be available in this callback. If there were any errors, the 
//		result will be set to the specific error code. Otherwise, the result 
//		will be set to SCResult_NO_ERROR.
//		<p />
//		See SCResult for error codes.
// See Also
...
// Returns
//		An SCResult enum value used to indicate the specific result of the 
//		request. This will return SCResult_NO_ERROR if the request completed 
//		successfully.
// Remarks
//		The function must be called in order to get a valid ATLAS SDK 
//		(SCInterface) object. Most other interface functions require an 
//		initialized interface object when being called. Note that if the 
//		standard GameSpy Availability Check was not performed prior to this 
//		call, the SDK will return SCResult_NO_AVAILABILITY_CHECK.
// See Also
//		scShutdown
...
//		successfully.
// Remarks
//		In order to clean up all resources used by the SDK, this interface 
//		function must be called once operations with this SDK are complete. Do 
//		not call this function if you plan to continue reporting stats.
// See Also
//		scInitialize
...
//		successfully.
// Remarks
//		This function should be called with a valid interface object. It will 
//		take care of pending requests that have been made by the interface 
//		functions.
// See Also
...
//								completes.
//		timeoutMs		: [in] The amount of time in milliseconds to wait for 
//								this operation to complete before timing out.
...
// scCreateSession
// Summary
//		Requests that the ATLAS service create a new game session and keep 
//		track of the session that is about to start.
// Parameters
//		theInterface	: [in] A valid ATLAS SDK object
//		certificate		: [in] A valid certificate obtained from the GameSpy 
//								AuthService.
...
//		successfully.
// Remarks
//		The certificate and private data may be NULL if the local client is an 
//		unauthenticated dedicated server. The function should be called by the 
//		host after initializing the SDK, and obtaining a certificate and 
//		private data from the authentication service. The competition service 
//		creates and sends a session ID to the host.	The callback passed in 
//		will get called even if the request failed.
// See Also
...
// Summary
//		This is a variation of scCreateSession that creates a "matchless" 
//		session. A matchless session isn't sanity-checked against other 
//		session data for arbitration of any statistics. It essentially turns 
//		off detection for cheating.	As data is sent in, it will be immediately 
//		applied to the stats, instead of being applied when the match has 
//		finished. A "matchless" game may be more commonly known as 
//		non-arbitrated or unranked.
// Parameters
//		theInterface	: [in] A valid ATLAS SDK object.
//		certificate		: [in] Certificate obtained from the GameSpy 
//								AuthService.
//		privateData		: [in] Private Data obtained from the GameSpy 
//								AuthService.
//		callback		: [in] The callback function to call when this request 
//								completes.
//		timeoutMs		: [in] The amount of time in milliseconds to wait for 
...
//		successfully.
// Remarks
//		Reports sent for matchless sessions should be marked as such using 
...
// Summary
//		Called to tell ATLAS the type of report that the player or host will 
//		send.
// Parameters
//		theInterface	: [ref] A valid ATLAS SDK object.
//		theConnectionId	: [in]  The player's connection id. Set to NULL unless 
//								the player is rejoining a session he previously 
//								left.
//		isAuthoritative	: [in]  Flag set if the snapshot being reported will be 
//								authoritative.
//		certificate		: [ref] Certificate obtained from the authentication 
//								web service.
//		privateData		: [ref] Private data obtained from the authentication 
//								web service.
//		callback		: [ref] The callback function to call when this request 
//								completes.
//		timeoutMs		: [in]  The amount of time in milliseconds to wait for 
//								this operation to complete before timing out.
//		userData		: [ref] Application data that may be used in the 
//								callback.
// Returns
//		An SCResult enum value used to indicate the specific result of the 
//		request. This will return SCResult_NO_ERROR if the request completed 
//		successfully.
// Remarks
//		The should be called by both the host and client before sending a 
//		report.
//		<p />
...
//		timeoutMs		: [in] The amount of time in milliseconds to wait for 
//								this operation to complete before timing out.
//		userData		: [in] Application data that may be used in the 
//								callback.
// Returns
//		An SCResult enum value used to indicate the specific result of the 
//		request. This will return SCResult_NO_ERROR if the request completed 
//		successfully.
// Remarks
//		Once the report has been completed with a call to scReportEnd, the 
//		player or host can call this function to submit a report. The 
//		certificate and private data are both required to submit a report.
//		Incomplete reports will be discarded. The callback passed in will tell 
//		the game the result of the operation.
// See Also
...
//		successfully.
// Remarks
//		The session ID identifies a single game session between players. 
//		Players should use the scGetSessionId function in order to obtain the 
//		session ID. This should not be called if a session has not yet been 
//		created.
// See Also
...
//		obtain the session GUID. The host then sends the session GUID to all 
//		other participants in the game session.
// See Also
...
// Remarks
//		The connection id identifies a single player in a game session. It may 
//		be possible to have different connection ids during the same session 
//		since players can come and leave sessions.
...
// Parameters
//		theInterface		: [in] A valid ATLAS SDK object.
//		theRulesetVersion	: [in] Ruleset version of the report as specified 
//									on the Web Admin Panel.
//		thePlayerCount		: [in] Player count for allocating enough resources 
//									and verification purposes.
//		theTeamCount		: [in] Team count for allocating enough resources 
//									and verification purposes.
...
//		that point for dedicated server games. This function should also be 
//		called before calling any scReport* function. The header version can be 
//		obtained from the ATLAS Web Admin Panel where the the keys are created.
//		<p />
// See Also
...
//		key-value functions.
//		<p />
//		ATLAS reports must include Global data first using 
//		scReportBeginGlobalData, then Player data using 
//		scReportBeginPlayerData, and finally Team data using 
//		scReportBeginTeamData. All three must be included in this order, even 
...
//		thePlayerIndex			: [in]	Index of the player (0 - Number of 
//										players).
//		thePlayerConnectionId	: [in]	Connection GUID generated by the ATLAS 
//										service and retrieved using 
//										scGetConnectionId().
//		thePlayerTeamIndex		: [in]	Team index of the player, if that 
//										player is on a team.
//		result					: [in]	Standard SC Game result.
//		theProfileId			: [in]	Profile ID of the player.
//		certificate				: [in]	Certificate obtained from the GameSpy 
//										AuthService. Note: This parameter is 
//										unused currently.
...
//		successfully.
// Remarks
//		A report must have been created prior to using this function. Each 
//		player must have a valid login certificate from the authentication 
//		service also. This function should be called after a new player is 
//		added to the report. Any key/value pairs that need to be added should 
...
//		successfully.
// Remarks
//		After the beginning of any team data is set, this function can be 
//		called to start a new team. After this function has been called, the 
//		game can start adding team data to the report.
// See Also
...
//		successfully.
// Remarks
//		Used to set the end of a report. The report must have been properly 
//		created and have some data. Any report being submitted requires that 
//		function be called before the submission. Incomplete reports will be 
//		discarded.
// See Also
...
// scReportSetAsMatchless
// Summary
//		Called after creating the report to set it as a matchless report; this 
//		is needed if the report is being submitted to a "matchless" game 
//		session. A "matchless" game may be more commonly known as 
//		non-arbitrated or unranked.
// Parameters
//		theReport	: [ref] A valid SC Report Object.
// Returns
...
//		successfully.
// Remarks
//		The host or player can call this function to add either global, 
//		player-, or team-specific data. A report needs to be created before 
//		calling this function. For global keys, this function can only be 
//		called after starting global data. For player or teams, a new player or 
...
//		successfully.
// Remarks
//		This should be called regardless of whether or not the report was 
//		submitted successfully. It should only be used if the report object 
//		contains a valid pointer from a successful call to scCreateReport.
// See Also
//		scCreateReport
...
//								This should point to NULL when passed.
//		queryParamsCount: [in]	The number of parameters that will be added.
// Returns
//		Enum value used to indicate the specific result of the call. This will 
//		return SCResult_NO_ERROR if the call completed successfully.
// Remarks
//		This needs to be called before performing any query function other than 
//		game queries. Any invalid input to the function will return 
//		SCResult_INVALID_PARAMETERS.  The user must pass in a queryParamsCount 
//		large enough to add the parameters required by the query (as listed on 
//		the Web Admin Panel). 
//		The function will return a failure code if the queryParams points to a 
//		non-NULL object. When the query has completed, it is the user's 
//		responsibility to clean up the parameter list.<p>
// See Also
...
//		return SCResult_NO_ERROR if the call completed successfully.
// Remarks
//		Parameters need to be added to a SCQueryParameterList before passing a 
//		SCQueryParameterList to a query function. This function should be used 
//		to add those parameters while not going	over the number of parameters 
//		preallocated in scCreateQueryParameterList. Keep in mind that incorrect 
//		parameter names or values are not going to be handled here. The query 
...
// Parameters
//		response		: [out] The pointer to the response passed into the 
//								callback.
// Returns
//		Enum value used to indicate the specific result of the call. This will 
//		return SCResult_NO_ERROR if the call completed successfully.
// Remarks
//		Use this function to destroy and NULL out an SCGameStatsQueryResponse. 
//		This call will only free a non-NULL response.<p>
// See Also
...
//		return SCResult_NO_ERROR if the call completed successfully.
// Remarks
//		Use this function to destroy and NULL out an 
//		SCPlayerStatsQueryResponse. This call will only free a non-NULL 
//		response.<p>
// See Also
//		SCPlayerStatQueryCallback
...
// Remarks
//		Use this function to destroy and NULL out an 
//		scDestroyTeamStatsQueryResponse. This call will only free a non-NULL 
//		response.<p>
// See Also
...
//		httpResult			: [in] http result used to describe lower level 
//									problems.
//		result				: [in] Result specifying a problem with the query.
//		msg					: [in] Pointer to text message explaining the 
//									result.
//		msgLen				: [in] The length of msg.
//		response			: [in] A pointer to the stats response that can be 
//									traversed.
//		userData			: [in/out] Application data that may be used in the 
//										callback.
// Returns
//		Enum value used to indicate the specific result of the call. This will 
//		return SCResult_NO_ERROR if the call completed successfully.
// Remarks
//		The user needs to implement this function signature so that the name 
//		of the function can be passed to scRunGameStatsQuery. Destroying the 
//		response is the user's responsibility. 
//		The parameter msg is for debugging purposes only; it should never be 
//		displayed to the player.<p>
// See Also
//		scRunGameStatsQuery, scDestroyGameStatsQueryResponse
typedef void (*SCGameStatsQueryCallback)(
	const SCInterfacePtr		theInterface,
	GHTTPResult					httpResult,
	SCResult					result,
	gsi_char					* msg,
...
// Parameters
//		theInterface		: [in]	A valid SCInterfacePtr.
//		certificate			: [in]	Pointer to GSLoginCertificate obtained from 
//									authservice.
//		privData			: [in]	Pointer to GSLoginPrivateData obtained from 
//									authservice.
//		ruleSetVersion		: [in]	Ruleset version number created by the user 
//									on the Web Admin Panel.
//		queryId				: [in]	A guid generated after building a query on 
//									the Web Admin Panel; you can use the 
//									variable defined in the auto-generated 
//									header file rather than pass the guid 
//									manually.
//		queryParameters		: [in]	SCQueryParameterListPtr that was built 
//									with parameters obtained from the Web Admin 
//									Panel.
//		callback			: [in]	User-supplied callback that must be valid 
//									in order to receive stats.
//		userData			: [in/out] Application data that may be used in the 
//										callback.
// Returns
//		Enum value used to indicate the specific result of the call. This will 
//		return SCResult_NO_ERROR if the call completed successfully.
// Remarks
//		Queries must first be built using the Web Admin Panel for ATLAS. Once 
//		a query has been generated, the user can take advantage of this generic 
//		function for game stats. The user must pass in valid parameters or the 
//		following error will be returned: SCResult_INVALID_PARAMETERS. The user 
//		should be calling the scThink function in order for this request to 
//		complete.<p>
// See Also
//		scCreateQueryParameterList
...
//		the function can be passed to scRunPlayerStatsQuery. Destroying the 
//		response is the user's responsibility. The parameter msg is for 
//		debugging purposes only; it should never be displayed to the player.<p>
// See Also
//		scRunPlayerStatsQuery, scDestroyPlayerStatsQueryResponse
typedef void (*SCPlayerStatsQueryCallback)(
	const SCInterfacePtr		theInterface, 
	GHTTPResult					httpResult, 
	SCResult					result, 
	gsi_char					* msg,
...
//									the Web Admin Panel; you can use the 
//									variable defined in the auto-generated 
//									header file rather than pass the guid manually.
//		queryParameters		: [in]	SCQueryParameterListPtr that was built with 
//									parameters obtained from the Web Admin Panel.
//		callback			: [in]	User-supplied callback that must be valid in 
//									order to receive stats.
//		userData			: [in/out] Application data that may be used in the 
//										callback.
// Returns
//		Enum value used to indicate the specific result of the call. This will 
...
//		Queries must first be built using the Web Admin Panel for ATLAS. Once
//		a query has been generated, the user can take advantage of this generic 
//		function for player stats. This function can be used for both single 
//		and multiple players or for a ranked leaderboard. The user must pass in 
//		valid parameters or the following error will be returned: 
//		SCResult_INVALID_PARAMETERS. The user should be calling the scThink 
//		function in order for this request to complete.<p>
// See Also
//		scCreateQueryParameterList
...
//		httpResult			: [in] http result used to describe lower-level 
//									problems.
//		result				: [in] Result specifying a problem with the 
//									query.
//		msg					: [in] Pointer to text message explaining the 
//									result.
//		msgLen				: [in] The length of msg.
...
//		is the user's responsibility. The parameter msg is for debugging 
//		purposes only; it should never be displayed to the player.<p>
// See Also
//		scRunTeamStatsQuery, scDestroyTeamStatsQueryResponse
typedef void (*SCTeamStatsQueryCallback)(
	const SCInterfacePtr		theInterface,
	GHTTPResult					httpResult,
	SCResult					result,
	gsi_char					* msg,
...
//									header file rather than pass the guid 
//									manually.
//		queryParameters		: [in]	SCQueryParameterListPtr which was built 
//									with parameters obtained from the Web 
//									Admin Panel.
//		callback			: [in]	User-supplied callback that must be valid 
//									in order to receive stats.
//		userData			: [in/out] Application data that may be used in the 
...
//		return SCResult_NO_ERROR if the call completed successfully.
// Remarks
//		Queries must first be built using the Web Admin Panel for ATLAS. Once a 
//		query has been generated, the user can take advantage of this generic 
//		function for player stats. This function can be used for both single 
//		and multiple players. The user must pass in valid parameters or the 
//		following error will be returned: SCResult_INVALID_PARAMETERS.<p>
// See Also
...
//		A symmetric cipher key for peer-to-peer communication. This will usually 
//		have one key for sending and a second key for receiving.
...
SCResult SC_CALL scPeerCipherInit(
...
// Encrypt/Decrypt in place, also the RC4 context is modified every time 
// encryption and decryption take place.
SCResult SC_CALL scPeerCipherEncryptBuffer(
	SCPeerCipher	* theCipher, 
	gsi_u8			* theData, 
	gsi_u32			theLen
);

SCResult SC_CALL scPeerCipherDecryptBuffer(
	SCPeerCipher	* theCipher, 
	gsi_u8			* theData, 
	gsi_u32			theLen
);

// When using UDP (non-ordered) you must supply a message num. This is less 
// efficient than encrypting an ordered stream.
SCResult SC_CALL scPeerCipherEncryptBufferIV(
	SCPeerCipher	* theCipher, 
	gsi_u32			theMessageNum, 
	gsi_u8			* theData, 
	gsi_u32			theLen);

SCResult SC_CALL scPeerCipherDecryptBufferIV(
	SCPeerCipher	* theCipher, 
	gsi_u32			theMessageNum, 
	gsi_u8			* theData, 
	gsi_u32			theLen
