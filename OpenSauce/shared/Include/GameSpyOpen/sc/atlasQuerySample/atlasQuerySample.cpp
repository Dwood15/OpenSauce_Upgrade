...
#include "../atlasSampleCommon/atlas_atlasSamples_v2.h"
#include "QueryData.h"
...
extern "C" void SetupStatsData(QueryData &queryStats)
{
	...
	queryStats.gameName = (gsi_char *) _T("atlasSamples");
	...
	queryStats.email = (gsi_char *) _T("sctest@gamespy.com");
	queryStats.profile = (gsi_char *) _T("sctest01");
	queryStats.password = (gsi_char *) _T("gspy");
	...
	queryStats.query1ParamCount = 1;
 	queryStats.query1ParamName = (gsi_char *) _T("profileid"); 
 	queryStats.query1ParamValue = (gsi_char *) _T("64880026");  // "<profileid num>"
 	sprintf(queryStats.query1QueryID, "%s", ATLAS_Query_Player_Stats_PROFILE_STATS_PLAYER);

	// query parameters for retrieving a leaderboard of top 10 records sorted by WIN_LOSS_RATIO
	queryStats.query2ParamCount = 2;
 	queryStats.query2Param1Name = (gsi_char *) _T("pageindex");
 	queryStats.query2Param1Value = (gsi_char *) _T("1");
 	queryStats.query2Param2Name = (gsi_char *) _T("pagesize");  
 	queryStats.query2Param2Value = (gsi_char *) _T("10"); 
	sprintf(queryStats.query2QueryID, "%s",ATLAS_Query_Player_Stats_PLAYER_LB);
	...
	queryStats.query3Param1Name = (gsi_char *) _T("pageindex");
	queryStats.query3Param1Value = (gsi_char *) _T("2");
	queryStats.query3Param2Name = (gsi_char *) _T("pagesize");  
	queryStats.query3Param2Value = (gsi_char *) _T("10"); 
	sprintf(queryStats.query3QueryID,"%s", ATLAS_Query_Player_Stats_PLAYER_LB);

	// query parameters for retrieving the game stats
	queryStats.query4ParamCount = 0;
	sprintf(queryStats.query4QueryID,"%s", ATLAS_Query_Game_Stats_METRICS_GLOBAL);
...
extern "C" int RunThroughSampleQueries(std::vector<std::string> &args)
{
	...
	GSIStartAvailableCheck(myQueryData.gameName);
...
#if (defined _UNIX || defined _PS3 || defined _PSP2) && !defined _IPHONE
