...
static void GSMD5Transform PROTO_LIST ((UINT4 [4], unsigned char [64]));
static void Encode PROTO_LIST ((unsigned char *, UINT4 *, unsigned int));
static void Decode PROTO_LIST ((UINT4 *, unsigned char *, unsigned int));
static void GSMD5_memcpy PROTO_LIST ((POINTER, POINTER, unsigned int));
static void GSMD5_memset PROTO_LIST ((POINTER, int, unsigned int));
...
)
{
  ...
  if (inputLen >= partLen) {
    GSMD5_memcpy ((POINTER)&context->buffer[index], (POINTER)input, partLen);
    GSMD5Transform (context->state, context->buffer);
  
    for (i = partLen; i + 63 < inputLen; i += 64)
      GSMD5Transform (context->state, &input[i]);
  ...
  )
{
  ...
  GSMD5_memset ((POINTER)context, 0, sizeof (*context));
}

// MD5 basic transformation. Transforms state based on block.
static void GSMD5Transform (UINT4 state[4], unsigned char block[64])
{
  ...
  // Zeroize sensitive information.
  GSMD5_memset ((POINTER)x, 0, sizeof (x));
}
...
static void GSMD5_memcpy (POINTER output, POINTER input, unsigned int len)
{
...
static void GSMD5_memset (POINTER output, int value, unsigned int len)
{
