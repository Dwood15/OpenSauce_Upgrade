...
#pragma warning( pop )
	...
	namespace Render
	{
		...
		static byte old_bytes[255];

		static void PLATFORM_API RenderWindowHookReflectionHook(const uint16 local_player_index
			, void* render_camera
			, void* render_frustum
...
#endif
			YELO_DEBUG_FORMAT("Writing window hook");
			Memory::WriteRelativeCall(&RenderWindowHookReflectionHook, GET_FUNC_VPTR(RENDER_WINDOW_REFLECTION_CALL), false);
			Memory::WriteRelativeJmp(&blam::render_window, CAST_PTR(void*, 0x50F8C8), true);
		}
