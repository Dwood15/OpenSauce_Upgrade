...
#pragma endregion
		...
		void		c_gbuffer_debug_effect::Render(IDirect3DDevice9* device, int16 debug_target)
		{
			...
			m_effect->SetTechnique(debug_target == NONE ? m_technique_all : m_technique_single);
...
skip_disable_velocity:
		...
		void		c_gbuffer_system::Initialize()
		{
			...
			Memory::WriteRelativeCall(&DrawIndexedPrimitive_Object, call_address, true);
			...
			Memory::WriteRelativeCall(&DrawIndexedPrimitive_Structure, call_address, true);
			Memory::WriteMemory(call_address + 5, &NOP, sizeof(NOP));
		}
		...
		void		c_gbuffer_system::AllocateResources(IDirect3DDevice9* pDevice, D3DPRESENT_PARAMETERS* params)
		{
			...
			m_multi_rt.count = (DX9::D3DCaps()->NumSimultaneousRTs > k_maximum_multi_render_target ? k_maximum_multi_render_target : DX9::D3DCaps()->NumSimultaneousRTs);
