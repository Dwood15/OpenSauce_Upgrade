...
namespace Yelo {
	namespace Interface { 
		namespace OpenSauceUI { 
			namespace Control {
		c_control_base::c_control_base(i_control& parent) : m_parent(parent), m_resource_id(RESOURCE_ID_NONE), m_child_controls(), m_property_interfaces(), m_event_handlers() { }
		...
		control_list_t::const_iterator c_control_base::FindControl(const control_ptr_t control) const {
			...
			return std::find_if(m_child_controls.begin(), m_child_controls.end(), [&](const control_ptr_t& entry) { return entry == control; }
			);
		}
		...
		i_property_interface* c_control_base::FindPropertyInterface(const uint32 interface_id) const {
			...
			if(found_interface == m_property_interfaces.end()) { return nullptr; }
...
#pragma region i_control
		i_control* c_control_base::Parent() { return &m_parent; }

		uint32 c_control_base::GetResourceID() const { return m_resource_id; }

		void c_control_base::SetResourceID(const uint32 resource_id) { m_resource_id = resource_id; }
		...
		void c_control_base::AddControl(control_ptr_t control) {
			...
			if(found_entry == m_child_controls.end()) { m_child_controls.push_back(control); }
		}
		...
		void c_control_base::RemoveControl(control_ptr_t control) {
			...
			if(found_entry != m_child_controls.end()) { m_child_controls.erase(found_entry); }
		}

		control_list_t& c_control_base::Controls() { return m_child_controls; }
