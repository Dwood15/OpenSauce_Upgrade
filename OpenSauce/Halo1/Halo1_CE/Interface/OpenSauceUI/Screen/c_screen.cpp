...
namespace Yelo { namespace Interface { namespace OpenSauceUI { namespace Screen {
		c_screen::c_screen(Control::control_ptr_t root_control) : m_root_control(root_control) , m_control_resource_list() { AddControlResource(root_control); }

#pragma region i_screen
		Control::control_ptr_t c_screen::GetRootControl() const { return m_root_control; }
...
#pragma endregion
		...
		void c_screen::AddControlResource(Control::control_ptr_t control) {
			...
			if(control->GetResourceID() != RESOURCE_ID_NONE) { m_control_resource_list[control->GetResourceID()] = control; }

			// Add the controls children to the resource list if appropriate
			for(auto& child : control->Controls()) { AddControlResource(child); }
		}

		void c_screen::Show() { GetRootControl()->Show(); }

		void c_screen::Hide() { GetRootControl()->Hide(); }
	};};};
};
#endif