...
#pragma region c_event_handler_gwen_handler
				void c_event_handler_gwen_handler::Callback(Gwen::Event::Info& info) {
					...
					if (m_data_transform) { m_data_transform(info, event_data); }

					m_owner.Trigger(event_data);
				}

				c_event_handler_gwen_handler::c_event_handler_gwen_handler(const c_event_handler_gwen& owner, Gwen::Event::Caller& caller
				...
				c_event_handler_gwen_handler::~c_event_handler_gwen_handler() { Detach(); }
...
#pragma region c_event_handler_gwen
				c_event_handler_gwen::c_event_handler_gwen(Gwen::Event::Caller& caller, const event_data_transform_t& data_transform) : Control::c_event_handler(), m_handler_thunk(*this, caller, data_transform) { }

				void c_event_handler_gwen::Attach() { m_handler_thunk.Attach(); }

				void c_event_handler_gwen::Detach() { m_handler_thunk.Detach(); }
#pragma endregion
			};
		};
	};
